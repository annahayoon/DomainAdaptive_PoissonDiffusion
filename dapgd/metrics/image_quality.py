...
def compute_niqe(
    ...
    PURPOSE: No-reference perceptual quality metric based on natural scene statistics
    ...
    try:
        ...
        raise ImportError("pyiqa package required. Install: pip install pyiqa")

    # Initialize NIQE model
    ...
    if pred_clamped.shape[1] == 1:
        pred_clamped = pred_clamped.repeat(1, 3, 1, 1)

    # Compute NIQE
    with torch.no_grad():
        niqe_scores = []
        for i in range(pred_clamped.shape[0]):
            niqe_val = niqe_metric(pred_clamped[i:i+1].to(device))
            niqe_scores.append(niqe_val.item())
...
class ImageQualityMetrics:
    ...
    def compute_all(
        self, prediction: torch.Tensor, target: torch.Tensor, compute_lpips: bool = True, compute_niqe: bool = False
    ) -> Dict[str, float]:
        """
        ...
        Args:
            ...
            compute_niqe: Whether to compute NIQE (no-reference metric)
        ...
        # NIQE (no-reference metric, optional)
        if compute_niqe:
            metrics["niqe"] = compute_niqe(prediction, device=self.device)
    ...
    def __call__(
        ...
        for metric in metrics:
            if metric == "psnr":
                ...
                result["niqe"] = compute_niqe(prediction, device=self.device)
            elif metric == "mse":
