...
class EDMPTDataset:
    ...
    def _find_paired_files(self) -> Tuple[List[Path], Dict[Path, Path]]:
        ...
        for tile in split_tiles:
            ...
            # From metadata (if it stores pt_path and it's not empty)
            metadata_pt_path = tile.get("pt_path", "")
            if metadata_pt_path and metadata_pt_path != "":
                # Convert absolute paths to relative paths based on current data_root
                if metadata_pt_path.startswith("/home/jilab/Jae/"):
                    # Convert Jae path to DAPGD path
                    relative_path = metadata_pt_path.replace("/home/jilab/Jae/dataset/processed/pt_tiles/", "")
                    pt_path_candidates.append(self.data_root.parent / relative_path)
                else:
                    pt_path_candidates.append(Path(metadata_pt_path))
            
            # Replace .png with .pt in png_path
            png_path = tile.get("png_path", "")
            if png_path and png_path != "":
                pt_path_candidates.append(Path(str(png_path).replace(".png", ".pt")))
            
            # Construct from data_root
            pt_path_candidates.extend([
    ...
    def _load_pt_image_direct(self, image_path: Path) -> torch.Tensor:
        ...
        try:
            ...
            tensor = torch.load(str(image_path), map_location='cpu', weights_only=True)
