...
class PNGDataset(Dataset):
    ...
    This dataset loads PNG images and converts them to the format expected
    by the diffusion model training pipeline, including noise simulation.
    ...
    def __init__(
        ...
        # Find PNG files
        self.image_files = self._find_png_files()
        ...
        logger.info(f"Loaded {len(self.split_files)} PNG files for {split} split")

    def _find_png_files(self) -> List[Path]:
        """Find all PNG files in the data directory."""
        png_files = []
        
        # Look for PNG files recursively
        for pattern in ["*.png", "*.PNG"]:
            png_files.extend(self.data_root.rglob(pattern))
        
        if not png_files:
            raise FileNotFoundError(f"No PNG files found in {self.data_root}")
    ...
    def _setup_transforms(self) -> transforms.Compose:
        ...
        transform_list = [
            ...
            transforms.Normalize(mean=[0.5] * self.channels, std=[0.5] * self.channels),  # Convert to [-1, 1] range
        ]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """
        Load and process a single PNG image.
        ...
        """
        try:
            image_path = self.split_files[idx]
            
            # Load clean image
            clean = self._load_png_image(image_path)
            ...
            return {
                'clean': torch.zeros(self.channels, self.image_size, self.image_size) - 1.0,  # [-1, 1] range
                'noisy': torch.zeros(self.channels, self.image_size, self.image_size) - 1.0,  # [-1, 1] range
                'electrons': torch.zeros(self.channels, self.image_size, self.image_size),     # [0, inf] range
