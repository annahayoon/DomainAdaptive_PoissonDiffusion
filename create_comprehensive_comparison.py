...
def normalize_for_display(img: np.ndarray, percentile_range: Tuple[float, float] = (1, 99)) -> np.ndarray:
...
def create_comprehensive_comparison():
    ...
    single_domain_paths = {
        "photography": base_path / "test_photography_single_domain" / "example_00_photography_sony_00135_00_0.1s_tile_0005",
        "microscopy": base_path / "test_microscopy_single_domain" / "example_00_microscopy_ER_Cell_002_RawGTSIMData_level_06_tile_0002",
        "astronomy": base_path / "test_astronomy_single_domain" / "example_00_astronomy_j8g6z3jdq_g800l_sci_tile_0071"
    }
    
    # Cross-domain results
    cross_domain_paths = {
        "photography": base_path / "test_photography_cross_domain" / "example_00_photography_sony_00135_00_0.1s_tile_0005",
        "microscopy": base_path / "test_microscopy_cross_domain" / "example_00_microscopy_ER_Cell_002_RawGTSIMData_level_06_tile_0002",
        "astronomy": base_path / "test_astronomy_cross_domain" / "example_00_astronomy_j8g6z3jdq_g800l_sci_tile_0071"
    }
    ...
    for domain, path in single_domain_paths.items():
        if path.exists():
            single_domain_results[domain] = load_results_from_json(path / "results.json")
    ...
    for domain in domains:
        ...
        summary_text += f"  PG x0 - Single: SSIM={single_ssim:.3f}, PSNR={single_psnr:.1f}dB\n"
        summary_text += f"  PG x0 - Cross:  SSIM={cross_ssim:.3f}, PSNR={cross_psnr:.1f}dB\n"
        summary_text += f"  Difference:     SSIM={ssim_diff:+.3f}, PSNR={psnr_diff:+.1f}dB\n\n"
    ...
    # Add column headers
    fig.text(0.5, 0.95, "Cross-Domain Low-Light Enhancement: Single-Domain vs Cross-Domain Models", 
             fontsize=16, fontweight='bold', ha='center')
    
    # Column labels
    fig.text(0.05, 0.88, "Clean\nReference", fontsize=12, fontweight='bold', ha='center')
    fig.text(0.25, 0.88, "Single-Domain Model", fontsize=12, fontweight='bold', ha='center')
    fig.text(0.75, 0.88, "Cross-Domain Model", fontsize=12, fontweight='bold', ha='center')
    ...
    for domain in domains:
        ...
        for method in methods:
            single_metrics = single_domain_results[domain]['comprehensive_metrics'][method]
            cross_metrics = cross_domain_results[domain]['comprehensive_metrics'][method]
            ...
            print(f"    Single-Domain: SSIM={single_ssim:.3f}, PSNR={single_psnr:.1f}dB")
