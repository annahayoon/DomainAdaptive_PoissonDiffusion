...
class PoissonGaussianGuidance(nn.Module):
    ...
    def _validate_inputs(self, x0_hat: torch.Tensor, y_e: torch.Tensor):
        ...
        assert (
            x0_hat.shape == y_e.shape
        ), f"Shape mismatch: {x0_hat.shape} vs {y_e.shape}"
        assert x0_hat.device == y_e.device, "Device mismatch"
        assert (
            x0_hat.min() >= 0.0 and x0_hat.max() <= 1.0
        ), f"x0_hat must be in [0,1], got [{x0_hat.min():.3f}, {x0_hat.max():.3f}]"
...
def calculate_exposure_ratio(
    noisy_filename: str, clean_filename: str, domain: str
) -> float:
    ...
    if noisy_exp is not None and clean_exp is not None and clean_exp > 0:
        ...
        logger.info(
            f"  Calculated exposure_ratio: α = {noisy_exp}s / {clean_exp}s = {alpha:.6f}"
        )
        return alpha
    else:
        logger.warning(
            f"  Could not extract exposures from {noisy_filename} / {clean_filename}"
        )
...
class UnifiedDenoiser:
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if i < num_steps - 1:
                ...
                if pg_guidance is not None and y_e is not None:
                    ...
                    pg_grad_next_norm = pg_guidance(
                        denoised_next_norm, y_e, t_next.item()
                    )
    ...
    def method_3_dps_guidance(
        ...
        logger.info(
            f"Method 3: DPS guidance with scale={scale:.1f}, weight={guidance_weight:.1f}"
        )
        ...
        for i in range(len(t_steps) - 1):
            ...
            with torch.no_grad():
                sigma_t = (
                    ...
                    else torch.tensor(t_curr, device=x.device)
                )
                if sigma_t.ndim == 0:
...
def find_corresponding_clean_reference(
    tile_id: str, domain: str, clean_dir: Path
    ...
    for exposure_time in exposure_times:
        ...
        if clean_path.exists():
            ...
            if (
                clean_tensor.ndim == 4 and clean_tensor.shape[1] == 3
            ):  # RGB image (B, C, H, W)
                clean_tensor = (
                    0.299 * clean_tensor[:, 0:1]
                    + 0.587 * clean_tensor[:, 1:2]
                    + 0.114 * clean_tensor[:, 2:3]
                )
            elif (
                clean_tensor.ndim == 3 and clean_tensor.shape[0] == 3
            ):  # RGB image (C, H, W)
                clean_tensor = (
                    0.299 * clean_tensor[0:1]
                    + 0.587 * clean_tensor[1:2]
...
def create_comprehensive_comparison(
    ...
    noisy_reference_scale = (
        noisy_scale_min,
        noisy_scale_max,
    )  # For dark/enhancement methods

    # ISSUE 2 FIX: Check if display is grayscale (1 channel) or RGB (3 channels)
    is_grayscale_display = noisy_display.ndim == 2 or (
        noisy_display.ndim == 3 and noisy_display.shape[2] == 1
    )
    ...
    if clean_image is not None:
        ...
        (
            ...
            clean_scale_min,
            clean_scale_max,
        ) = prepare_for_display(clean_phys)

        # Store clean reference scale for bright methods
        clean_reference_scale = (clean_scale_min, clean_scale_max)

        is_grayscale_clean = clean_display.ndim == 2 or (
            clean_display.ndim == 3 and clean_display.shape[2] == 1
        )
        axes[panel_idx].imshow(
            clean_display, cmap="gray" if is_grayscale_clean else None
        )
    ...
    for i, method_name in enumerate(method_names):
        ...
        (
            ...
            res_scale_min,
            res_scale_max,
        ) = prepare_for_display(result_phys, scale_range=scale_to_use)

        # ISSUE 2 FIX: Check if result display is grayscale or RGB
        is_grayscale_result = result_display.ndim == 2 or (
            result_display.ndim == 3 and result_display.shape[2] == 1
        )
        axes[panel_idx + i].imshow(
            result_display, cmap="gray" if is_grayscale_result else None
        )
        ...
        if clean_image is not None:
            ...
            if clean_for_metrics.shape == result_for_metrics.shape:
                metrics = compute_metrics(clean_for_metrics, result_for_metrics)
    ...
    # Main title
    comparison_type = (
        "Low-Light Enhancement" if clean_image is None else "Denoising Comparison"
    )
    plt.suptitle(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Unified denoising methods comparison")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory containing clean reference .pt files",
    )
    ...
    # Output arguments
    parser.add_argument(
        "--output_dir", type=str, default="results/unified_denoising_comparison"
    )
    parser.add_argument(
        "--num_examples",
        type=int,
    ...
    for tile_info in test_tiles:
        ...
        if noisy_path.exists():
            ...
            clean_ref = find_corresponding_clean_reference(
                tile_id, args.domain, Path(args.clean_dir)
            )
            if clean_ref is not None:
                available_tiles.append(tile_info)

    logger.info(
        f"Found {len(available_tiles)} tiles with noisy files and corresponding clean references"
    )
    ...
    for idx, tile_info in enumerate(selected_tiles):
        ...
        try:
            ...
            noisy_image = load_image(
                tile_id, Path(args.noisy_dir), denoiser.device, "noisy"
            )

            # Find corresponding clean reference
            clean_image = find_corresponding_clean_reference(
                tile_id, args.domain, Path(args.clean_dir)
            )
            ...
            logger.info(
                f"  Brightness: {noisy_brightness['category']} (mean={noisy_brightness['mean']:.3f})"
            )
            ...
            if noisy_tile_id:
                clean_ref = find_corresponding_clean_reference(
                    noisy_tile_id, args.domain, Path(args.clean_dir)
                )
                if clean_ref is not None:
            ...
            if noisy_tile_id and clean_tile_id:
                exposure_ratio = calculate_exposure_ratio(
                    noisy_tile_id, clean_tile_id, args.domain
                )
            else:
                logger.warning(
                    f"  Could not determine clean tile_id for {noisy_tile_id}, using default α=0.01"
                )
            ...
            if args.s is None:
                logger.info(
                    f"  PG Guidance: s auto-set to domain_max ({domain_max:.1f}), alpha={exposure_ratio:.6f}"
                )
    ...
    logger.info(
        f"🔬 Methods: Adaptive Sigma, PG Guidance, DPS Guidance, Synthetic Denoising"
    )
    logger.info("=" * 80)
