...
def load_image(tile_id: str, image_dir: Path, device: torch.device, image_type: str = "image") -> torch.Tensor:
    ...
    logger.debug(f"Loaded {image_type} image: {tile_id}, shape={tensor.shape}, range=[{tensor.min():.3f}, {tensor.max():.3f}]")
...
def load_clean_image(tile_id: str, clean_dir: Path, device: torch.device) -> torch.Tensor:
...
class EDMLowLightEnhancer:
    ...
    def enhance_low_light(
        ...
        """
        logger.info(f"Enhancing low-light image with sigma={enhancement_sigma:.3f}, CFG scale={cfg_scale:.1f}")
        ...
        logger.info(f"Enhancement steps: {t_steps[0]:.3f} -> {t_steps[-1]:.3f} ({len(t_steps)-1} steps)")
        ...
        logger.info(f"✓ Enhancement completed: range [{enhanced_output.min():.3f}, {enhanced_output.max():.3f}]")
    ...
    def optimize_sigma(
        ...
        """
        logger.info(f"Optimizing sigma in range [{sigma_range[0]:.6f}, {sigma_range[1]:.6f}] with {num_trials} trials")
        ...
        logger.info(f"✓ Best sigma: {best_sigma:.6f} ({metric}={best_metric_value:.4f})")
...
def create_enhancement_comparison(
    ...
    noisy_display, noisy_min, noisy_max, noisy_p1, noisy_p99, scale_min, scale_max = prepare_for_display(noisy_phys)
    ...
    for i, sigma in enumerate(sigmas):
        ...
        enhanced_phys = denormalize_to_physical(enhanced_tensor, domain)
        enhanced_display, enh_min, enh_max, enh_p1, enh_p99, _, _ = prepare_for_display(
            enhanced_phys, 
            scale_range=reference_scale  # Use same scale as noisy input
        )
        
        axes[i + 1].imshow(enhanced_display, cmap='gray' if enhanced_display.ndim == 2 else None)
        axes[i + 1].set_title(
    ...
    logger.info(f"✓ Enhancement comparison saved: {save_path} (physical units with common scale [{scale_min:.1f}, {scale_max:.1f}] {unit_label})")
...
def main():
    ...
    parser.add_argument("--noise_method", type=str, default="local_var",
    ...
    parser.add_argument("--use_adaptive_sigma", action="store_true",
    ...
    for idx, tile_info in enumerate(selected_tiles):
        ...
        try:
            ...
            if args.use_adaptive_sigma:
                ...
                logger.info(f"    Mean brightness: {adaptive_info['mean_brightness']:.6f}")
                logger.info(f"    Darkness factor: {adaptive_info['darkness_factor']:.6f}")
                logger.info(f"    Adaptive σ: {adaptive_info['adaptive_sigma']:.6f}")
                logger.info(f"    Noise-relative σ: {adaptive_info['noise_relative_sigma']:.6f}")
            else:
            ...
            if args.optimize_sigma:
                ...
                clean_image = load_clean_image(tile_id, Path(args.clean_dir), enhancer.device)
                ...
                clean_np = clean_image.cpu().numpy()
                metrics = compute_metrics(clean_np, enhanced_np)
                logger.info(f"  Final metrics: SSIM={metrics['ssim']:.4f}, PSNR={metrics['psnr']:.2f}dB")
            else:
                # Use estimated sigma
            ...
            for sigma, enhanced in enhancement_results.items():
                torch.save(enhanced.cpu(), sample_dir / f"enhanced_sigma_{sigma:.7f}.pt")
    ...
    logger.info(f"📊 Processed {len(all_results)} tiles with per-tile sigma determination")
    if args.optimize_sigma and len(all_results) > 0:
        logger.info(f"📈 Mean SSIM: {summary['aggregate_metrics']['mean_ssim']:.4f} ± {summary['aggregate_metrics']['std_ssim']:.4f}")
        logger.info(f"📈 Mean PSNR: {summary['aggregate_metrics']['mean_psnr']:.2f} ± {summary['aggregate_metrics']['std_psnr']:.2f} dB")
    logger.info("=" * 80)
