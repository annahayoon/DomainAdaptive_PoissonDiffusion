...
class CrossDomainOptimizer:
    ...
    def select_random_tiles(self, args: Dict[str, Any], num_tiles: int = 50) -> List[str]:
    ...
    def collect_tiles_from_all_domains(self, args: Dict[str, Any], tiles_per_domain: Dict[str, int] = None) -> Dict[str, List[str]]:
        ...
        for domain_key, num_tiles in tiles_per_domain.items():
            if domain_key.startswith("photography_"):
                ...
                domain_args["noisy_dir"] = "dataset/processed/pt_tiles/photography/noisy"
                domain_args["clean_dir"] = "dataset/processed/pt_tiles/photography/clean"
    ...
    def create_unified_parameter_space(self, args: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        param_combinations = list(itertools.product(kappa_range, sigma_r_range, num_steps_range))
    ...
    def load_and_save_clean_tile(self, tile_id: str, args: Dict[str, Any], tile_output_dir: str) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            target_channels = 3 if args["domain"] in ["astronomy", "microscopy"] else None
            clean_image, clean_metadata = self.load_clean_image(clean_tile_id, clean_dir, device, target_channels)
    ...
    def optimize_domain(self, domain: str, sensor: str = "default") -> Dict[str, Any]:
        ...
        print(f"\n{'='*80}")
        print(f"🔍 Optimizing Cross-Domain Parameters: {domain.upper()} ({sensor.upper()})")
        print(f"{'='*80}")
    ...
    def run_unified_cross_domain_sweep(self, args: Dict[str, Any], all_tiles: Dict[str, List[str]]) -> List[Dict[str, Any]]:
        ...
        print(f"Testing {len(param_combinations)} parameter combinations across all domains...")
        ...
        for i, (kappa, sigma_r, num_steps) in enumerate(param_combinations):
            ...
            for domain_key, tiles in all_tiles.items():
                ...
                for tile_id in tiles:
                    ...
                    tile_result = self.run_inference_single_tile(tile_args, tile_id, output_dir)
            ...
            if all_tile_results:
                ...
                aggregated_results = self.aggregate_tile_results(all_tile_results, output_dir)
    ...
    def save_unified_results(self, results: List[Dict[str, Any]], best_result: Dict[str, Any]):
    ...
    def print_unified_summary(self, results: List[Dict[str, Any]], best_result: Dict[str, Any]):
        ...
        if best_result:
            ...
            print(f"   κ={best_result['kappa']}, σ_r={best_result['sigma_r']}, steps={best_result['num_steps']}")
        ...
        for i, result in enumerate(sorted_results, 1):
            print(f"   {i}. κ={result['kappa']}, σ_r={result['sigma_r']}, steps={result['num_steps']} | Score: {result['score']:.4f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cross-domain parameter optimization for cross-domain low-light enhancement")
    
    # Required arguments
    parser.add_argument("--model_path", type=str, default="results/edm_pt_training_multi_domain_20251010_182807/best_model.pkl", 
    ...
    # Optional arguments
    parser.add_argument("--num_examples", type=int, default=60, help="Number of example images to test (distributed across domains)")
    ...
    parser.add_argument("--output_base", type=str, default="results/cross_domain_optimization",
                       help="Base output directory for results")
    parser.add_argument("--kappa_range", type=float, nargs=3, default=[0.1, 0.5, 0.9],
                       help="Kappa values to test (min, mid, max)")
    parser.add_argument("--sigma_r_range", type=float, nargs=3, default=[1.0, 3.0, 5.0],
                       help="Sigma_r values to test (min, mid, max)")
    parser.add_argument("--num_steps_range", type=int, nargs=3, default=[15, 25, 35],
