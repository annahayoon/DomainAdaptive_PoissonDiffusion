...
def extract_exposure_ratio(
    tile_id: str, clean_dir: Optional[Path] = None
) -> Optional[float]:
    """
    ...
    try:
        ...
        if clean_dir is not None:
            ...
            if clean_exp is not None:
                ...
                logger.info(
                    f"  Extracted α = {short_exp}s / {clean_exp}s = {alpha:.4f}"
                )
                return alpha
...
def find_clean_exposure(tile_id: str, clean_dir: Path) -> Optional[float]:
    ...
    try:
        ...
        if tile_pattern_parts:
            ...
            logger.debug(
                f"  Searching for clean reference with pattern: {tile_pattern}"
            )
...
def load_image(
    ...
    if "astronomy" in tile_id.lower() and tensor.min() < 0:
        ...
        if current_max > 0:
            ...
            logger.debug(
                f"Scaled astronomy {image_type} image {tile_id}: range [0, {current_max:.3f}] -> [0, {tensor.max():.3f}]"
            )

        logger.debug(
            f"Processed astronomy {image_type} image {tile_id}: shifted by {min_val:.3f}, final range [{tensor.min():.3f}, {tensor.max():.3f}]"
        )
        logger.debug(
            "Note: Negative values represent physically meaningful low-intensity regions after calibration corrections"
        )
    ...
    logger.debug(
        f"Loaded {image_type} image: {tile_id}, shape={tensor.shape}, range=[{final_min:.3f}, {final_max:.3f}]"
    )
    ...
    def __init__(
        ...
        self.domain_ranges = domain_ranges or {
            ...
            "astronomy": {
                "min": 0.0,
                "max": 450.0,
            },  # Fixed: removed negative min, increased max
        }
...
def main():
    ...
    if args.use_sensor_calibration:
        ...
        logger.warning(
            "  ⚠️  Consider --use_sensor_calibration for physics-based approach!"
        )
        ...
        try:
            ...
            if args.use_sensor_calibration:
                ...
                if args.domain == "photography":
                    ...
                    sensor_mapping = {"sony": "sony_a7s_ii", "fuji": "fuji_x_t2"}
                    sensor_name = sensor_mapping[
                        extracted_sensor
                    ]  # No fallback - must be found
                    logger.info(
                        f"  Using CALIBRATED sensor parameters (extracted: {extracted_sensor} -> {sensor_name})"
                    )
                else:
                    sensor_name = args.sensor_name
                    logger.info(
                        f"  Using CALIBRATED sensor parameters (sensor: {sensor_name})"
                    )
                ...
                logger.warning(
                    "  Consider using --use_sensor_calibration for physics-based approach!"
                )

                noise_estimates = NoiseEstimator.estimate_noise_comprehensive(
                    noisy_image
                )
                ...
                logger.info(
                    f"  Using {args.noise_method}: noise_σ={estimated_noise:.6f}"
                )
            ...
            if args.domain == "photography":
                ...
                if clean_pair_info:
                    ...
                    logger.warning(
                        f"  No clean pair found, using fallback α = {noisy_exposure}s / 30.0s = {exposure_ratio:.4f}"
                    )
                ...
                exposure_ratio = (
                    1.0 / flux_ratio
                )  # Convert flux ratio to exposure ratio for enhancement
                logger.info(
                ...
                logger.info(
                    f"  Enhancement factor: grism observations scaled by {exposure_ratio:.1f}x to match direct image brightness"
                )
    ...
    if "comprehensive_aggregate_metrics" in summary:
        ...
        if (
            ...
            and "aggregate_metrics" in summary
        ):
            logger.info(
                f"📈 Legacy Mean SSIM: {summary['aggregate_metrics']['mean_ssim']:.4f} ± {summary['aggregate_metrics']['std_ssim']:.4f}"
            )
            logger.info(
                f"📈 Legacy Mean PSNR: {summary['aggregate_metrics']['mean_psnr']:.2f} ± {summary['aggregate_metrics']['std_psnr']:.2f} dB"
            )
        logger.info("=" * 80)
