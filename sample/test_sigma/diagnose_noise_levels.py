...
def analyze_noise_levels(results_dir: Path, domain: str, clean_dir: Path = None):
    ...
    example_dirs = [
        ...
        if d.is_dir()
        and (d.name.startswith("example_") or d.name.startswith("sample_"))
    ]
    ...
    for example_dir in example_dirs:
        ...
        logger.info(
            f"  Value range: [{noisy_image.min():.6f}, {noisy_image.max():.6f}]"
        )
            ...
            if len(parts) >= 3:
                ...
                if clean_path.exists():
                    try:
                        ...
                        clean_analysis = analyze_image_noise(
                            clean_image, f"{example_dir.name}_clean"
                        )
                        all_clean_analysis[example_dir.name] = clean_analysis

                        logger.info(f"  Found clean tile: {clean_path.name}")
                        logger.info(
                            f"  Clean mean: {clean_image.mean():.6f}, Clean std: {clean_image.std():.6f}"
                        )
...
def analyze_image_noise(image: torch.Tensor, name: str) -> Dict:
    ...
    if image.shape[-1] > 64 and image.shape[-2] > 64:
        ...
        if image.ndim == 4:
            patch = image[
                0,
                :,
                center_h - patch_size // 2 : center_h + patch_size // 2,
    ...
    if image.shape[-1] > 1 and image.shape[-2] > 1:
        ...
        h_noise_std = h_diff.std().item() / np.sqrt(
            2
        )  # Divide by sqrt(2) for difference
    ...
    estimated_noise_std = min(
        estimated_noise_std, estimated_noise_std_hf, estimated_noise_std_diff
    )
    ...
    for i, (name, analysis) in enumerate(all_analysis.items()):
        ...
        if image.ndim == 4:
            img_display = (
                image[0, 0].cpu().numpy()
                if image.shape[1] == 1
                else image[0].mean(dim=0).cpu().numpy()
            )
        elif image.ndim == 3:
            img_display = (
                image[0].cpu().numpy()
                if image.shape[0] == 1
                else image.mean(dim=0).cpu().numpy()
            )
        else:
        ...
        axes[1, i].text(
            0.1,
            0.7,
            f"Est. Noise: {analysis['estimated_noise_std']:.6f}",
            transform=axes[1, i].transAxes,
            fontsize=10,
        )
        axes[1, i].text(
        ...
        axes[1, i].text(
            0.1,
            0.4,
            f"HF Noise: {analysis['estimated_noise_std_hf']:.6f}",
            transform=axes[1, i].transAxes,
            fontsize=10,
        )
        axes[1, i].text(
            0.1,
            0.3,
            f"Diff Noise: {analysis['estimated_noise_std_diff']:.6f}",
            transform=axes[1, i].transAxes,
            fontsize=10,
        )
...
def create_histogram_visualization(
    all_analysis: Dict, all_clean_analysis: Dict, results_dir: Path
):
    ...
    for i, (name, analysis) in enumerate(all_analysis.items()):
        ...
        if has_clean and name in all_clean_analysis:
            ...
            ax4.hist(
                clean_img_flat, bins=100, alpha=0.7, edgecolor="black", color="green"
            )
            ax4.axvline(
                clean_analysis["mean"],
                color="darkgreen",
            ...
            ax4.axvline(
                clean_analysis["mean"] - clean_analysis["std"],
                color="olive",
                linestyle=":",
                linewidth=1.5,
                label=f"±1σ",
            ...
            ax4.set_title(
                f"Clean Tile Pixel Distribution", fontsize=12, fontweight="bold"
            )
            ...
            if clean_image.shape[-1] > 1 and clean_image.shape[-2] > 1:
                if clean_image.ndim == 4:
                    clean_img_2d = (
                        clean_image[0, 0].cpu().numpy()
                        if clean_image.shape[1] == 1
                        else clean_image[0].mean(dim=0).cpu().numpy()
                    )
                elif clean_image.ndim == 3:
                    clean_img_2d = (
                        clean_image[0].cpu().numpy()
                        if clean_image.shape[0] == 1
                        else clean_image.mean(dim=0).cpu().numpy()
                    )
                else:
                ...
                ax5.axvline(
                    -clean_h_diff.std(), color="darkgreen", linestyle="--", linewidth=2
                )
                ax5.set_xlabel("Pixel-to-Pixel Difference (Clean)", fontsize=11)
                ax5.set_ylabel("Frequency", fontsize=11)
                ax5.set_title(
                    f"Clean Tile Noise Distribution", fontsize=12, fontweight="bold"
                )
                ...
                if h_diff is not None:
                    noise_reduction = (
                        (h_diff.std() - clean_h_diff.std()) / h_diff.std() * 100
                    )
                    ax5.text(
                        0.98,
                        0.95,
                        f"Noise Δ: {noise_reduction:.1f}% reduction\nNoisy: {h_diff.std():.6f}\nClean: {clean_h_diff.std():.6f}",
...
def provide_recommendations(all_analysis: Dict):
    ...
    logger.info(
        f"  Estimated noise std: {min_noise:.6f} - {max_noise:.6f} (avg: {avg_noise:.6f})"
    )
    logger.info(
        f"  Signal-to-noise ratio: {min_snr:.2f} - {max_snr:.2f} dB (avg: {avg_snr:.2f} dB)"
    )
    ...
    if avg_noise < 0.0001 and min_noise < 0.0001:
        logger.info(
            "⚠ Current sigma range (0.000001 - 0.01) may be too high for these clean images"
        )
        logger.info("✓ Consider testing much lower sigma values (0.0000001 - 0.000001)")
    elif avg_noise > 0.01:
        logger.info(
            "⚠ Current sigma range (0.000001 - 0.01) may be too low for these noisy images"
        )
...
def main():
    ...
    if clean_dir and not clean_dir.exists():
        logger.warning(
            f"Clean directory not found: {clean_dir}, proceeding without clean comparison"
        )
        clean_dir = None
