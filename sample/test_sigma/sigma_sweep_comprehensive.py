...
def load_noisy_image(tile_id: str, noisy_dir: Path, device: torch.device) -> torch.Tensor:
    ...
    logger.debug(f"Loaded noisy image: {tile_id}, shape={noisy_tensor.shape}, range=[{noisy_tensor.min():.3f}, {noisy_tensor.max():.3f}]")
...
class EDMDenoiser:
    ...
    def denoise(
        ...
        logger.info(f"Time steps: {t_steps[0]:.6f} -> {t_steps[-1]:.6f} ({len(t_steps)-1} steps)")
...
def create_comprehensive_comparison(
    ...
    axes[0, 0].imshow(norm_display_model(noisy_image), cmap='gray' if noisy_image.shape[1] == 1 else None)
    axes[0, 0].set_title(f"Noisy Input\nRange: [{noisy_image.min():.3f}, {noisy_image.max():.3f}]", fontsize=9)
    axes[0, 0].axis('off')
    ...
    for i, sigma in enumerate(sigmas):
        ...
        axes[0, i + 1].imshow(denoised_display, cmap='gray' if denoised_display.ndim == 2 else None)
        axes[0, i + 1].set_title(f"σ = {sigma:.6f}\nRange: [{denoised_tensor.min():.3f}, {denoised_tensor.max():.3f}]", fontsize=9)
        axes[0, i + 1].axis('off')
    ...
    for i, sigma in enumerate(sigmas):
        ...
        denoised_display, denoised_min, denoised_max = norm_display_percentile(denoised_physical)
        
        axes[1, i + 1].imshow(denoised_display, cmap='gray' if denoised_display.ndim == 2 else None)
        axes[1, i + 1].set_title(f"σ = {sigma:.6f}\nRange: [{denoised_min:.1f}, {denoised_max:.1f}]", fontsize=9)
        axes[1, i + 1].axis('off')
    
    # Add row labels
    axes[0, 0].set_ylabel("Model Space\n[-1, 1]", fontsize=10, fontweight="bold")
    axes[1, 0].set_ylabel("Physical Units\n+ Display Norm", fontsize=10, fontweight="bold")
...
def main():
    ...
    # Output arguments
    parser.add_argument("--output_dir", type=str, default="results/comprehensive_sigma_sweep")
    parser.add_argument("--num_examples", type=int, default=3, help="Number of example images to test")
    ...
    if len(available_tiles) == 0:
        logger.error(f"No tiles found with corresponding noisy files in {args.noisy_dir}")
        return
    ...
    sigma_values = [0.0000001, 0.0000002, 0.0000005, 0.000001, 0.000002, 0.000005, 0.00001, 0.00002, 0.00005, 0.0001]
    ...
    for idx, tile_info in enumerate(selected_tiles):
        ...
        try:
            # Load noisy image
            noisy_image = load_noisy_image(tile_id, Path(args.noisy_dir), denoiser.device)
            if noisy_image.ndim == 3:
                noisy_image = noisy_image.unsqueeze(0)
            noisy_image = noisy_image.to(torch.float32)
            
            logger.info(f"  Noisy range: [{noisy_image.min():.4f}, {noisy_image.max():.4f}], std={noisy_image.std():.4f}")
            ...
            comparison_path = output_dir / f"example_{idx:02d}_{tile_id}_sigma_sweep.png"
            try:
            ...
            for sigma, denoised in sigma_results.items():
                torch.save(denoised.cpu(), sample_dir / f"denoised_sigma_{sigma:.6f}.pt")
    ...
    logger.info(f"📊 Tested {len(sigma_values)} sigma values on {len(selected_tiles)} examples")
    logger.info(f"🎯 Each example shows: noisy input → denoised outputs across sigma range")
    logger.info("=" * 80)
