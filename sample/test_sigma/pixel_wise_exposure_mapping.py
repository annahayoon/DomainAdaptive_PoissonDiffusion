...
def analyze_pixel_wise_mapping(num_tiles=10):
    ...
    try:
        ...
        logger.info(f"Polynomial: H = {a3:.4f}·L³ + {b3:.4f}·L² + {c3:.4f}·L + {d3:.4f}")
    ...
    try:
        ...
        popt_var, pcov_var = curve_fit(linear_var, bin_centers, bin_vars,
                                        p0=[const_var, 0.0],
        ...
        if p_value < 0.05:
            if alpha > 0:
                logger.info(f"    ✓ HETEROSCEDASTIC: variance increases with L (Poisson-like)")
            else:
    ...
    create_pixelwise_visualization(
        L, H, bin_centers, bin_means, bin_stds, bin_vars,
        popt_lin, const_var, sigma2_read if var_fitted else 0,
        alpha if var_fitted else 0, r2_const, r2_lin_var if var_fitted else 0,
        best_var_model, correlation,
    ...
    results = {
        ...
        'mean_function': {
            ...
            'polynomial': {
                'params': [float(a3), float(b3), float(c3), float(d3)] if poly_fitted else None,
                'r2': float(r2_poly) if poly_fitted else None,
                'rmse': float(rmse_poly) if poly_fitted else None
            } if poly_fitted else None
        },
        'variance_function': {
            'type': best_var_model,
...
def create_pixelwise_visualization(
    L, H, bin_centers, bin_means, bin_stds, bin_vars,
    popt_lin, const_var, sigma2_read, alpha,
    r2_const, r2_lin_var, best_var_model, correlation,
    output_path
):
    ...
    hist2d, xedges, yedges = np.histogram2d(
        L[idx_sample], H[idx_sample], bins=150,
        range=[[L.min(), L.max()], [H.min(), H.max()]]
    )
    
    # Plot as heatmap
    extent = [xedges[0], xedges[-1], yedges[0], yedges[-1]]
    im = ax1.imshow(hist2d.T, origin='lower', extent=extent, aspect='auto',
                    cmap='Blues', interpolation='gaussian', alpha=0.9, zorder=5)
    ...
    a, b = popt_lin
    H_mean = a * L_plot + b
    ax1.plot(L_plot, H_mean, 'r-', linewidth=1.5, 
            label=rf'$\mu(L) = {a:.2f} \cdot L + {b:.2f}$', zorder=10)
    ...
    ax1.set_title('(a) Pixel-Wise Exposure Mapping: Low-Light to High-Light',
                 fontsize=15, fontweight='bold', loc='left', pad=10)
    ax1.legend(fontsize=11, loc='upper left', frameon=True, framealpha=0.95, 
    ...
    ax2.scatter(bin_centers, bin_means, s=100, alpha=0.8,
                edgecolors='black', linewidth=1.5, color='#2E86AB',
                label='Binned means', zorder=5)
    ...
    if len(bin_means) > 0:
        ...
        ax2.set_xlim([bin_centers.min() - 0.02*x_range, bin_centers.max() + 0.02*x_range])
    ...
    ax2.text(0.95, 0.05, rf'$R^2$ = {r2_mean:.4f}', transform=ax2.transAxes,
    ...
    ax3.scatter(bin_centers, bin_vars, s=120, alpha=0.9, 
               edgecolors='black', linewidth=2, color='#A23B72',
    ...
    r2_const = 1 - (ss_res_const / ss_tot) if ss_tot > 0 else 0
    ax3.axhline(const_var, color='#1f77b4', linestyle='--', linewidth=2.5,
               label=f'Constant: $R^2$ = {r2_const:.3f}', alpha=0.9, zorder=3)
    ...
    if r2_lin_var > -np.inf and alpha != 0:
        ...
        ax3.plot(L_plot, var_linear_plot, color='#d62728', linestyle='-', linewidth=2.5,
                label=f'P-G: $R^2$ = {r2_pg:.3f}', alpha=0.9, zorder=4)
        ...
        ax3.text(0.97, 0.97, comparison_text, 
                transform=ax3.transAxes, fontsize=9, verticalalignment='top',
                horizontalalignment='right',
                bbox=dict(boxstyle='round', facecolor=best_color, alpha=0.85, 
                         edgecolor=edge_color, linewidth=2.5))
    else:
        # Only constant model available - upper right corner
        ax3.text(0.97, 0.97, f"Best: Constant\n$R^2$ = {r2_const:.3f}", 
                transform=ax3.transAxes, fontsize=9, verticalalignment='top',
                horizontalalignment='right',
    ...
    ax3.set_title('(c) Variance Model Comparison', fontsize=13, fontweight='bold', loc='left')
    ax3.legend(fontsize=9, loc='upper right', frameon=True, framealpha=0.95, 
              edgecolor='black', fancybox=False)
    ...
    if len(bin_vars) > 0:
        x_range = bin_centers.max() - bin_centers.min()
        ax3.set_xlim([bin_centers.min() - 0.02*x_range, bin_centers.max() + 0.02*x_range])
        
        # Set equal aspect ratio for square scaling
        ax3.set_aspect('equal', adjustable='box')
    
    plt.suptitle('Pixel-Wise Exposure Mapping: Statistical Analysis',
...
def main():
    ...
    parser.add_argument("--num_tiles", type=int, default=10,
                       help="Number of tile pairs to analyze")
