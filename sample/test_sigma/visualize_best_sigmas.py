...
def create_comparison_visualization(samples: List[Dict], output_path: Path):
    ...
    for i, sample in enumerate(samples):
        ...
        axes[i, 1].imshow(
            norm_display(enhanced_0008),
            cmap="gray" if enhanced_0008.shape[1] == 1 else None,
        )
        axes[i, 1].set_title(
            f"Enhanced σ=0.0008\nRange: [{enhanced_0008.min():.4f}, {enhanced_0008.max():.4f}]",
            fontsize=10,
        )
        axes[i, 1].axis("off")
        ...
        axes[i, 2].imshow(
            norm_display(enhanced_002),
            cmap="gray" if enhanced_002.shape[1] == 1 else None,
        )
        axes[i, 2].set_title(
            f"Enhanced σ=0.002\nRange: [{enhanced_002.min():.4f}, {enhanced_002.max():.4f}]",
            fontsize=10,
        )
        axes[i, 2].axis("off")
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize enhancement results for best sigma values"
    )
    ...
    selected_tiles = random.sample(
        available_tiles, min(args.num_samples, len(available_tiles))
    )
