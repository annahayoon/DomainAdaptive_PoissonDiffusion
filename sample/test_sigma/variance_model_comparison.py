...
def compute_binned_variance(L, H, n_bins=150, min_count=200):
    ...
    return (np.array(bin_centers), np.array(bin_variances), 
            np.array(bin_counts), np.array(bin_means))
...
def fit_variance_model(model_func, bin_L, bin_var, bin_counts, p0, bounds=(-np.inf, np.inf)):
    """Fit a variance model with weighted least squares."""
    try:
        weights = np.sqrt(bin_counts)
        popt, pcov = curve_fit(
            model_func, bin_L, bin_var,
            p0=p0, bounds=bounds,
        ...
        log_likelihood = -n/2 * np.log(2*np.pi) - n/2 * np.log(sigma_resid**2) - np.sum(residuals**2)/(2*sigma_resid**2)
...
def compare_variance_models(num_tiles=30):
    ...
    mask = ((np.abs(L) > max(L_threshold_low, L_min_threshold)) & 
    ...
    logger.info(f"Removed {(~mask).sum():,} pixels ({100*(~mask).sum()/len(mask):.2f}%)")
    ...
    bin_L, bin_var, bin_counts, bin_means = compute_binned_variance(L, H, n_bins=150, min_count=200)
    ...
    models['constant'] = fit_variance_model(
        constant_var, bin_L, bin_var, bin_counts,
        p0=[const_var], bounds=(0, np.inf)
    )
    ...
    models['linear'] = fit_variance_model(
        linear_var, bin_L, bin_var, bin_counts,
        p0=[const_var, 0.0], bounds=(0, np.inf)
    )
    ...
    models['quadratic'] = fit_variance_model(
        quadratic_var, bin_L, bin_var, bin_counts,
        p0=[const_var, 0.0, 0.0]
    )
    ...
    models['sqrt'] = fit_variance_model(
        sqrt_var, bin_L, bin_var, bin_counts,
        p0=[const_var, 0.0], bounds=(0, np.inf)
    )
    ...
    models['exponential'] = fit_variance_model(
        exponential_var, bin_L, bin_var, bin_counts,
        p0=[const_var, 0.0]
    )
    ...
    create_variance_comparison_viz(
        bin_L, bin_var, bin_counts,
        models, best_model_name, L,
...
def create_variance_comparison_viz(bin_L, bin_var, bin_counts, models, best_model, L, output_path):
    ...
    ax1.plot(bin_L, bin_var, 'o-', color='black', markersize=4, linewidth=1.5, 
            alpha=0.7, label='Empirical Var($H|L$)', zorder=5)
    
    # Add subtle shading to show data density (lighter approach)
    # Normalize counts for color intensity
    norm_counts = (bin_counts - bin_counts.min()) / (bin_counts.max() - bin_counts.min())
    for i in range(len(bin_L)-1):
        ax1.fill_between([bin_L[i], bin_L[i+1]], 
    ...
    for name, model in models.items():
        if model['success']:
            ...
            ax1.plot(L_plot, var_plot, linestyle=linestyle, linewidth=linewidth,
                    color=colors[name], label=label_str, alpha=alpha)
    
    ax1.set_xlabel('Low-Light Intensity ($L$)', fontsize=15, fontweight='bold')
    ax1.set_ylabel('Var($H|L$)', fontsize=15, fontweight='bold')
    ax1.set_title('(a) Variance Models: Empirical vs Fitted', fontsize=17, fontweight='bold', pad=15)
    ax1.legend(fontsize=11, loc='best', framealpha=0.95, edgecolor='black', fancybox=False)
    ax1.grid(True, alpha=0.25, linewidth=1)
    ax1.tick_params(labelsize=12)
    
    # Add data point count annotation
    ax1.text(0.02, 0.98, f'$n$ = {len(bin_L)} bins\nTotal pixels: {len(L):,}',
    ...
    for idx, (name, model) in enumerate(successful_models[:5]):  # Up to 5 models
        ax = fig.add_subplot(gs[0, 3 + idx]) if idx < 3 else fig.add_subplot(gs[1, idx - 3 + 3])
        
        residuals = bin_var - model['predictions']
        # Use connected line with small markers instead of scattered points
        ax.plot(bin_L, residuals, 'o-', markersize=3, linewidth=1, 
               alpha=0.8, color=colors[name], zorder=5)
        ax.axhline(0, color='red', linestyle='--', linewidth=2.5, alpha=0.8)
        ax.fill_between([bin_L.min(), bin_L.max()], -2*np.std(residuals), 2*np.std(residuals),
        ...
        ax.text(0.95, 0.95, f'RMSE:\n{rmse:.4f}', transform=ax.transAxes,
    ...
    bars = ax_aic.barh(model_names, aics, color=model_colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ...
    bars = ax_bic.barh(model_names, bics, color=model_colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ...
    bars = ax_r2.barh(model_names, r2s, color=model_colors, alpha=0.8, edgecolor='black', linewidth=1.5)
    ...
    plt.suptitle('Rigorous Variance Model Comparison\nStatistical Evidence for Noise Structure',
