...
def create_combined_histogram(csv_path: Path, output_path: Path):
    ...
    target_sigmas = [0.0002, 0.0004, 0.0006, 0.0008, 0.001, 0.0012, 0.0014, 0.0016, 0.0018, 0.002]
    sigma_values = [s for s in all_sigma_values if any(abs(s - t) < 1e-9 for t in target_sigmas)]
    ...
    fig, axes = plt.subplots(n_metrics, n_sigmas, figsize=(2.8 * n_sigmas, 2.8 * n_metrics))
    ...
    for metric in metrics:
        if metric == 'psnr':
            ...
            best_metric_sigmas[metric] = sigma_summary['mse'].idxmin()  # Lower is better
        elif metric == 'enhanced_range' and 'enhanced_range' in sigma_summary.columns:
            best_metric_sigmas[metric] = sigma_summary['enhanced_range'].idxmax()  # Higher is better
        elif metric == 'enhanced_std' and 'enhanced_std' in sigma_summary.columns:
            best_metric_sigmas[metric] = sigma_summary['enhanced_std'].idxmax()  # Higher detail
        elif metric == 'enhanced_mean' and 'enhanced_mean' in sigma_summary.columns:
            best_metric_sigmas[metric] = sigma_summary['enhanced_mean'].idxmax()  # Brighter
    ...
    for i, metric in enumerate(metrics):
        for j, sigma in enumerate(sigma_values):
            ...
            n, bins, patches = ax.hist(data, bins=25, alpha=0.75, edgecolor='black', 
            ...
            if i == 0:
                ...
                ax.set_title(title_str, fontsize=11, fontweight='bold', pad=8, color=title_color)
    ...
    for i, metric in enumerate(metrics):
        axes[i, 0].text(-0.35, 0.5, metric_labels[metric], 
...
def main():
    parser = argparse.ArgumentParser(description="Create combined histogram visualization")
