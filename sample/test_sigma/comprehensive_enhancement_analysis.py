...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive enhancement analysis on all test tiles"
    )
...
def create_histograms(df: pd.DataFrame, sigma_values: List[float], output_dir: Path):
    ...
    for metric in metrics:
        ...
        plt.suptitle(
            f"{metric_labels[metric]} Distribution Across All Test Tiles",
            fontsize=16,
            fontweight="bold",
        )
        plt.tight_layout()
...
def recommend_best_sigma(df: pd.DataFrame, output_dir: Path):
    ...
    logger.info(
        f"Best PSNR: σ = {best_psnr_sigma:.4f} ({sigma_summary.loc[best_psnr_sigma, 'psnr']:.2f} dB)"
    )
    logger.info(
        f"Best SSIM: σ = {best_ssim_sigma:.4f} ({sigma_summary.loc[best_ssim_sigma, 'ssim']:.4f})"
    )
    logger.info(
        f"Best Brightness Increase: σ = {best_brightness_sigma:.4f} ({sigma_summary.loc[best_brightness_sigma, 'brightness_increase']:.6f})"
    )

    # Balanced recommendation (60% brightness, 40% SSIM)
    normalized_brightness = (
        sigma_summary["brightness_increase"]
        - sigma_summary["brightness_increase"].min()
    ) / (
        sigma_summary["brightness_increase"].max()
        - sigma_summary["brightness_increase"].min()
        + 1e-8
    )
    ...
    logger.info(
        f"  - Brightness Increase: {sigma_summary.loc[best_balanced_sigma, 'brightness_increase']:.6f}"
    )
    logger.info(
        f"  - Enhanced Mean: {sigma_summary.loc[best_balanced_sigma, 'enhanced_mean']:.6f}"
    )
    ...
    recommendation = {
        ...
        "metrics": {
            ...
            "brightness_increase": float(
                sigma_summary.loc[best_balanced_sigma, "brightness_increase"]
            ),
            "enhanced_mean": float(
                sigma_summary.loc[best_balanced_sigma, "enhanced_mean"]
            ),
        },
    }
