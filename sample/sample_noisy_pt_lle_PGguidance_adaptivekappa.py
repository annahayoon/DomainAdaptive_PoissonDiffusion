...
class FIDCalculator:
    ...
    def _calculate_fid(self, features1: np.ndarray, features2: np.ndarray) -> float:
        ...
        if features1.shape[0] < 2 or features2.shape[0] < 2:
            logger.warning(
                f"Insufficient samples for FID: {features1.shape[0]}, {features2.shape[0]}"
            )
            return float("nan")
...
class GaussianGuidance(nn.Module):
    ...
    def __init__(
        ...
        if offset > 0:
            logger.info(
                f"✓ Astronomy offset applied: {offset:.3f} (correcting for calibration-induced negative values)"
            )
...
class PoissonGaussianGuidance(nn.Module):
    ...
    def __init__(
        ...
        # Log adaptive kappa initialization
        if self.use_adaptive_kappa:
            logger.info(f"AdaptiveKappaPGGuidance initialized:")
            logger.info(f"  Base kappa: {self.base_kappa}")
            logger.info(f"  Adaptive kappa: enabled")
            logger.info(f"  Kappa range: [{self.min_kappa}, {self.max_kappa}]")
            logger.info(f"  Signal threshold: {self.signal_threshold} electrons")
    ...
    def forward(
        ...
        # Apply adaptive kappa if enabled
        if self.use_adaptive_kappa:
            # Estimate signal level
            signal_level = self.estimate_signal_level(x0_hat)
            ...
            self.debug_info["base_kappa"] = self.base_kappa

        # Compute gradient
        gradient = self._compute_gradient(x0_hat, y_e)

        # Apply guidance with current kappa (adaptive or base)
        step_size = self.kappa * (sigma_t**2)
        x0_guided = x0_hat + step_size * gradient

        # Restore original kappa if adaptive was used
        if self.use_adaptive_kappa:
    ...
    for tile in tiles:
        if (
            ...
            and tile.get("tile_id", "").startswith(
                f"astronomy_{scene_id}_detection_sci_tile_"
            )
...
def find_clean_tile_pair_microscopy(
    ...
    logger.info(
        f"Extracted components - specimen: {specimen}, cell: {cell}, scene_id: {scene_id}, tile_id: {tile_id}"
    )
    ...
    for tile in tiles:
        if (
            ...
            and tile.get("data_type") == "clean"
            and tile.get("tile_id", "").startswith(
                f"microscopy_{specimen}_{cell}_{scene_id}_"
            )
            and tile.get("tile_id", "").endswith(f"_tile_{tile_id}")
        ):
            clean_candidates.append(tile)
    ...
    for tile in metadata["tiles"]:
        if tile["tile_id"] == noisy_tile_id:
            ...
            if "clean_path" in tile and tile["clean_path"]:
                logger.info(
                    f"Found LOLv2 clean pair: {noisy_tile_id} -> {tile['clean_path']}"
                )
                return tile
...
def compute_comprehensive_metrics(
    ...
    if not METRICS_AVAILABLE or EvaluationSuite is None:
        ...
        return compute_simple_metrics(
            clean, enhanced, fid_calculator=fid_calculator, device=device
        )
    ...
    try:
        ...
        metrics = {
            ...
            "lpips": lpips_result.value
            if not np.isnan(lpips_result.value)
            else float("nan"),
            "niqe": niqe_result.value
            if not np.isnan(niqe_result.value)
            else float("nan"),
...
def compute_simple_metrics(
    ...
    try:
        ...
        psnr_val = (
            20
            * torch.log10(torch.tensor(1.0) / torch.sqrt(torch.tensor(mse_val))).item()
        )
    ...
    def optimize_sigma(
        ...
        for sigma in sigma_values:
            ...
            metrics = compute_simple_metrics(
                clean_image, restored, fid_calculator=fid_calculator, device=self.device
            )
...
def validate_exposure_ratio(noisy_tensor, clean_tensor, assumed_alpha, domain, logger):
    ...
    if error_percent > 20.0:
        ...
        logger.warning(
            f"    ⚠️  MODERATE ERROR: Measured α differs by {error_percent:.1f}%"
        )
    else:
    ...
    try:
        ...
        try:
            ssim_val = ssim(
                restored_np,
                clean_np,
                multichannel=True if restored_np.ndim == 3 else False,
                data_range=1.0,
                win_size=7,
            )
        except ValueError:
...
def main():
    ...
    parser.add_argument(
        "--use_adaptive_kappa",
        action="store_true",
        help="Enable adaptive kappa scheduling based on signal level",
    )
    parser.add_argument(
        "--min_kappa",
        type=float,
        default=0.05,
        help="Minimum kappa value for ultra-low signal regions",
    )
    parser.add_argument(
        "--max_kappa",
        type=float,
        default=2.0,
        help="Maximum kappa value for high signal regions",
    )
    parser.add_argument(
        "--signal_threshold",
        type=float,
        default=50.0,
        help="Signal threshold (electrons) for full kappa strength",
    )
    ...
    pg_guidance = PoissonGaussianGuidance(
        ...
        # Adaptive kappa parameters
        use_adaptive_kappa=args.use_adaptive_kappa,
    ...
    for idx, tile_info in enumerate(selected_tiles):
        ...
        try:
            ...
            if args.domain == "photography":
                ...
                exposure_ratio = 1.0
                logger.info(
                    f"  LOLv2: Using exposure_ratio=1.0 (images are already exposure-normalized)"
                )
                ...
                if clean_pair_info is None:
                    raise ValueError(
                        f"No clean tile pair found for {tile_id} - this should not happen for photography data"
                    )
                ...
                logger.info(
                    f"  Extracted α = {noisy_exposure}s / {clean_exposure}s = {exposure_ratio:.4f}"
                )
                ...
                logger.info(
                    f"  Microscopy enhancement scale: clean is 1.7x brighter → α={exposure_ratio:.4f}"
                )
                ...
                exposure_ratio = (
                    flux_ratio  # α = 0.35 (grism is underexposed compared to direct)
                )
                logger.info(
                    f"  Astronomy flux scale: g800l grism vs direct detection → α={exposure_ratio:.4f}"
                )
                logger.info(
                    f"  Note: Flux calibration ratio {flux_ratio:.2f} (direct/grism), calibrated using standard stars"
                )
                logger.info(
                    f"  Enhancement factor: grism observations scaled by {1/exposure_ratio:.1f}x to match direct image brightness"
                )
                logger.info(
                    f"  Note: α < 1 because grism is underexposed compared to direct imaging"
                )
            ...
            if args.validate_exposure_ratios and clean_image is not None:
                try:
                    ...
                    if error_percent > 20.0:
                        logger.error(
                            f"  CRITICAL: Exposure ratio mismatch detected! Consider updating hardcoded values."
                        )
                except Exception as e:
            ...
            if args.clean_dir is not None:
                ...
                if args.domain in ["photography", "photography_lolv2"]:
                    ...
                    if clean_pair_info:
                        ...
                        if Path(clean_pt_path).exists():
                            try:
                                clean_image_tensor, _ = load_clean_image(
                                    clean_tile_id,
                                    Path(args.clean_dir),
                                    sampler.device,
                                    target_channels=sampler.net.img_channels,
                                ...
                                logger.info(
                                    f"  Found clean reference via metadata: {clean_tile_id}"
                                )
                            except Exception as e:
                            ...
                            logger.warning(
                                f"  Clean tile file not found: {clean_pt_path}"
                            )
                            clean_image = None
                    else:
        ...
        if "gaussian_x0_cross" in args.run_methods or "pg_x0_cross" in args.run_methods:
            ...
            if args.domain in ["photography", "photography_lolv2"]:
                cross_domain_class_labels[
                    :, 0
                ] = 1.0  # Photography domain (includes LOLv2)
            elif args.domain == "microscopy":
        ...
        if "gaussian_x0_cross" in args.run_methods:
            ...
            if restored_gaussian_x0_cross.shape[1] == 3 and args.domain in [
                "astronomy",
                "microscopy",
            ]:
                restored_gaussian_x0_cross = restored_gaussian_x0_cross.mean(
                    dim=1, keepdim=True
                )
                logger.info(
                    "    Converted RGB output back to grayscale for metrics comparison"
                )
        ...
        if "pg_x0_cross" in args.run_methods:
            ...
            # Convert RGB output back to grayscale for astronomy/microscopy (original 1-channel domains)
            if restored_pg_x0_cross.shape[1] == 3 and args.domain in [
                "astronomy",
                "microscopy",
            ]:
        ...
        if "noisy" in args.run_methods:
            ...
            if noisy_for_results.shape[1] == 3 and args.domain in [
                "astronomy",
                "microscopy",
            ]:
        ...
        if "clean" in args.run_methods and clean_image is not None:
            ...
            if clean_for_results.shape[1] == 3 and args.domain in [
                "astronomy",
                "microscopy",
            ]:
        ...
        if clean_image is not None:
            ...
            if args.fast_metrics:
                ...
                logger.info(
                    "  Using fast metrics (PSNR, SSIM, MSE only) - skipping LPIPS/NIQE/FID"
                )
                metrics_results["noisy"] = compute_metrics_fast(
                    noisy_01, clean_01, sampler.device
                )
                logger.info(
                    f"    noisy: PSNR={metrics_results['noisy']['psnr']:.2f}dB, SSIM={metrics_results['noisy']['ssim']:.4f}"
                )
            else:
                try:
            ...
            for method, restored_tensor in restoration_results.items():
                ...
                # Convert back to grayscale for astronomy/microscopy for accurate metrics
                if restored_01.shape[1] == 3 and args.domain in [
                    "astronomy",
                    "microscopy",
                ]:
                ...
                try:
                    if args.fast_metrics:
                        # Use fast metrics for 5-10x speedup
                        metrics_results[method] = compute_metrics_fast(
                            restored_01, clean_01, sampler.device
                        )
                        logger.info(
                            f"    {method}: PSNR={metrics_results[method]['psnr']:.2f}dB, SSIM={metrics_results[method]['ssim']:.4f}"
                        )
                    else:
            ...
            if args.fast_metrics:
                ...
                metrics_results["clean"] = compute_metrics_fast(
                    clean_01, clean_01, sampler.device
                )
            else:
                try:
        ...
        try:
            ...
            for method in all_restored_images_by_method:
                if len(all_restored_images_by_method[method]) > 1:
                    ...
                    restored_batch = torch.stack(
                        all_restored_images_by_method[method], dim=0
                    )
                    ...
                    if method in summary["comprehensive_aggregate_metrics"]:
                        summary["comprehensive_aggregate_metrics"][method][
                            "fid"
                        ] = fid_score
    ...
    if "comprehensive_aggregate_metrics" in summary:
        ...
        if summary["comprehensive_aggregate_metrics"]:
            ...
            if "mean_lpips" in last_metrics and not np.isnan(
                last_metrics["mean_lpips"]
            ):
                logger.info(
                    f"    LPIPS: {last_metrics['mean_lpips']:.4f} ± {last_metrics['std_lpips']:.4f}"
                )

            if "mean_niqe" in last_metrics and not np.isnan(last_metrics["mean_niqe"]):
                logger.info(
                    f"    NIQE: {last_metrics['mean_niqe']:.4f} ± {last_metrics['std_niqe']:.4f}"
                )

            if "fid" in last_metrics and not np.isnan(last_metrics["fid"]):
                logger.info(f"    FID: {last_metrics['fid']:.4f}")

            if "mean_chi2_consistency" in last_metrics and not np.isnan(
                last_metrics["mean_chi2_consistency"]
            ):
                logger.info(
                    f"    χ²: {last_metrics['mean_chi2_consistency']:.4f} ± {last_metrics['std_chi2_consistency']:.4f}"
                )

            if "mean_residual_distribution" in last_metrics and not np.isnan(
                last_metrics["mean_residual_distribution"]
            ):
                logger.info(
                    f"    Res-KS: {last_metrics['mean_residual_distribution']:.4f} ± {last_metrics['std_residual_distribution']:.4f}"
                )
