...
class SensorCalibration:
    ...
    def get_sensor_params(domain: str, sensor_name: str = "generic") -> Dict[str, Any]:
        ...
        if sensor_name not in domain_sensors:
            print(
                f"Warning: Sensor '{sensor_name}' not found, using 'generic' for {domain}"
            )
            sensor_name = "generic"
    ...
    def compute_s_parameter(
        domain_min: float, domain_max: float, target_scale: float = 1000.0
    ) -> float:
        """
    ...
    def compute_sigma_r_normalized(
        read_noise_physical: float, domain_min: float, domain_max: float, s: float
    ) -> float:
        """
...
def main():
    ...
    print(
        f"   Sensor: {params['sensor_info']['sensor_model']} ({params['sensor_info']['dataset']})"
    )
    ...
    print(
        f"   QE: {params['sensor_info']['quantum_efficiency']*100:.0f}%, Gain: {params['sensor_info']['gain']:.2f} e-/ADU"
    )
    ...
    print(f"   Domain range: [{params['domain_min']}, {params['domain_max']}]")
    print(
        f"   Instrument: {params['sensor_info']['instrument']} ({params['sensor_info']['dataset']})"
    )
