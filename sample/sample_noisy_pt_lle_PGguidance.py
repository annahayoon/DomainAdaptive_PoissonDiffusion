...
def get_metadata_path_for_exposure_ratio(domain: str, base_metadata_json: Path) -> Path:
    ...
    domain_metadata_map = {
        ...
        "astronomy": base_path / "pt_tiles" / "astronomy_v2" / "metadata_astronomy_incremental.json",
    }
...
def calculate_exposure_ratio_from_metadata(
    ...
    if domain == "microscopy":
        ...
        raise ValueError(f"Exposure ratio calculation not implemented for domain: {domain}")
    ...
    for tile in tiles:
        ...
        if 'is_clean' in tile:
            ...
            if (tile.get('domain') == 'astronomy' and 
                tile.get('is_clean') == True and
                tile_id_str.startswith(f"astronomy_{scene_id}_") and
                tile_id_str.endswith(f"_tile_{tile_id}")):
                clean_candidates.append(tile)
        else:
            # V1 metadata: look for data_type='clean' and detection_sci
            if (tile.get('domain') == 'astronomy' and 
                tile.get('data_type') == 'clean' and
                tile_id_str.startswith(f"astronomy_{scene_id}_detection_sci_tile_") and
                tile_id_str.endswith(f"_tile_{tile_id}")):
                clean_candidates.append(tile)
...
def find_clean_tile_pair_microscopy(
    ...
    logger.info(f"Extracted components - specimen: {specimen}, middle: {middle_parts}, scene_id: {scene_id}, level: {level}, tile_id: {tile_id}")
...
def compute_simple_metrics(clean, enhanced, data_range: float = None, device: str = "cuda") -> Dict[str, float]:
    """
...
def find_and_load_clean_image(tile_id: str, domain: str, metadata_json: Path, sampler_device: torch.device, target_channels: Optional[int] = None) -> Tuple[Optional[torch.Tensor], Optional[Any]]:
    """
...
def load_clean_image_from_path(clean_pt_path: Path, device: torch.device, target_channels: Optional[int] = None) -> Tuple[torch.Tensor, Any]:
    """
...
def convert_range(tensor: torch.Tensor, from_range: str = "[-1,1]", to_range: str = "[0,1]") -> torch.Tensor:
...
def analyze_image_brightness(image: torch.Tensor) -> Dict[str, float]:
    ...
    brightness_category = (
        "Very Dark" if mean_brightness < 0.2 else
        "Dark" if mean_brightness < 0.4 else
        "Medium" if mean_brightness < 0.6 else
        "Bright" if mean_brightness < 0.8 else
        "Very Bright"
    )
        ...
        try:
            ...
            try:
                ...
                if 'numpy._core' in str(pickle_e) or 'scalar' in str(pickle_e):
                    ...
                    logger.error("")
                    logger.error("The photography model was saved with numpy 2.0+ which is incompatible")
                    logger.error("with Python 3.8 and numpy 1.24.4 in this environment.")
                    logger.error("")
                    logger.error("SOLUTIONS:")
                    logger.error("  1. Use Python 3.10+: /usr/bin/python3.10 or /usr/bin/python3.11")
                    logger.error("  2. Repackage the model on a numpy 1.x compatible system")
                    logger.error("  3. Contact the model provider for a compatible version")
                    logger.error("=" * 80)
                    raise RuntimeError(
    ...
    def posterior_sample(
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if pg_guidance is not None and y_e is not None:
                x_denoised_01 = torch.clamp(convert_range(x_denoised, "[-1,1]", "[0,1]"), 0.0, 1.0)
                
                if pg_guidance.guidance_level == 'x0':
                    x_guided = pg_guidance(x_denoised_01, y_e, t_cur.item())
                    x_denoised = torch.clamp(convert_range(x_guided, "[0,1]", "[-1,1]"), -1.0, 1.0)
            ...
            if not no_heun and i < num_steps - 1:
                ...
                if pg_guidance is not None and y_e is not None:
                    x_denoised_next_01 = torch.clamp(convert_range(x_denoised_next, "[-1,1]", "[0,1]"), 0.0, 1.0)

                    if pg_guidance.guidance_level == 'x0':
                        x_guided_next = pg_guidance(x_denoised_next_01, y_e, t_next.item())
                        x_denoised_next = torch.clamp(convert_range(x_guided_next, "[0,1]", "[-1,1]"), -1.0, 1.0)
                elif gaussian_guidance is not None and y_e is not None:
                    x_denoised_next_01 = torch.clamp(convert_range(x_denoised_next, "[-1,1]", "[0,1]"), 0.0, 1.0)
...
def filter_metrics_for_json(metrics_results: Dict[str, Dict[str, float]], domain: str = "photography") -> Dict[str, Dict[str, float]]:
    """
    ...
    for method, metrics in metrics_results.items():
        ...
        elif method in ['exposure_scaled', 'gaussian_x0', 'gaussian', 'pg_x0', 'pg', 'pg_score']:
            # Enhanced methods
            if is_photography:
                # Photography: SSIM, PSNR, LPIPS, NIQE
                filtered[method] = {
...
def validate_exposure_ratio(noisy_tensor, clean_tensor, assumed_alpha, domain, logger):
    ...
    if error_percent > 20.0:
        ...
        logger.warning(f"    MODERATE ERROR: Measured α differs by {error_percent:.1f}%")
    else:
...
def compute_metrics_fast(restored, clean, device):
    ...
    try:
        ...
        return {'psnr': float('nan'), 'ssim': float('nan'), 'mse': float('nan'), 'lpips': float('nan'), 'niqe': float('nan')}
    ...
    for idx, tile_info in enumerate(selected_tiles):
        ...
        try:
            ...
            if args.domain == "photography":
                ...
                microscopy_metadata_path = get_metadata_path_for_exposure_ratio(args.domain, Path(args.metadata_json))
                
                # Find clean tile pair
                clean_pair_info = find_clean_tile_pair(tile_id, microscopy_metadata_path, args.domain)
                ...
                astronomy_metadata_path = get_metadata_path_for_exposure_ratio(args.domain, Path(args.metadata_json))
                
                # Find clean tile pair
                clean_pair_info = find_clean_tile_pair(tile_id, astronomy_metadata_path, args.domain)
                if clean_pair_info is None:
                    raise ValueError(f"No clean tile pair found for {tile_id}")
            ...
            if args.validate_exposure_ratios and clean_image is not None:
                try:
                    ...
                    if error_percent > 20.0:
                        logger.error(f"  Exposure ratio mismatch detected! Consider updating hardcoded values.")
                except Exception as e:
            ...
            if args.clean_dir is not None:
                try:
                    logger.info(f"  Attempting to load clean image from: {args.clean_dir}")
            ...
            if clean_image is not None:
                logger.info(f"  ✅ Successfully loaded clean image: shape={clean_image.shape}, range=[{clean_image.min():.4f}, {clean_image.max():.4f}]")
            else:
                ...
                raise FileNotFoundError(f"Clean image not found for tile {tile_id}. Clean pairs MUST exist for validation!")
        ...
        if 'gaussian_x0_cross' in args.run_methods:
            ...
            restored_gaussian_x0_cross = ensure_grayscale_for_domain(restored_gaussian_x0_cross, args.domain)
        ...
        if 'pg_x0_cross' in args.run_methods:
            ...
            restored_pg_x0_cross = ensure_grayscale_for_domain(restored_pg_x0_cross, args.domain)
        ...
        if 'noisy' in args.run_methods:
            restoration_results['noisy'] = ensure_grayscale_for_domain(noisy_image, args.domain)
        if 'clean' in args.run_methods and clean_image is not None:
            restoration_results['clean'] = ensure_grayscale_for_domain(clean_image, args.domain)
        
        # Compute comprehensive metrics for all methods
        if clean_image is not None:
            clean_01 = torch.clamp(convert_range(clean_image, "[-1,1]", "[0,1]"), 0.0, 1.0)
            clean_01 = ensure_grayscale_for_domain(clean_01, args.domain)
            
            noisy_01 = torch.clamp(convert_range(noisy_image, "[-1,1]", "[0,1]"), 0.0, 1.0)
            ...
            try:
                ...
                metrics = metrics_results['noisy']
                logger.info(f"    noisy: SSIM={metrics['ssim']:.4f}, PSNR={metrics['psnr']:.2f}dB")
                if 'lpips' in metrics and not np.isnan(metrics['lpips']):
                    niqe_val = metrics.get('niqe', 'N/A')
                    niqe_str = f"{niqe_val:.4f}" if isinstance(niqe_val, (int, float)) else str(niqe_val)
            ...
            for method, restored_tensor in restoration_results.items():
                if restored_tensor is None:
                    continue
                restored_01 = torch.clamp(convert_range(restored_tensor, "[-1,1]", "[0,1]"), 0.0, 1.0)
                ...
                try:
                    ...
                    logger.debug(f"      LPIPS value: {lpips_debug}, NIQE value: {niqe_debug}")
        ...
        assert clean_image is not None, f"CRITICAL: clean_image is None but processing continued. This should not happen!"
        ...
        result_info = {
            ...
            'comprehensive_metrics': filter_metrics_for_json(metrics_results, domain=args.domain),  # Add filtered comprehensive metrics
    ...
    if 'comprehensive_aggregate_metrics' in summary:
        ...
        for method, method_metrics in summary['comprehensive_aggregate_metrics'].items():
            ...
            if method == 'noisy':
                ...
                # Only show LPIPS and NIQE for photography domain
                if args.domain == "photography":
                    if 'mean_lpips' in method_metrics and not np.isnan(method_metrics['mean_lpips']):
                        logger.info(f"    LPIPS: {method_metrics['mean_lpips']:.4f} ± {method_metrics['std_lpips']:.4f}")
                    if 'mean_niqe' in method_metrics and not np.isnan(method_metrics['mean_niqe']):
                        logger.info(f"    NIQE: {method_metrics['mean_niqe']:.4f} ± {method_metrics['std_niqe']:.4f}")
            else:
