...
class SingleDomainOptimizer:
    ...
    def select_random_tiles(
        self, args: Dict[str, Any], num_tiles: int = 1
    ) -> List[str]:
    ...
    def load_and_save_clean_tile(
        ...
        try:
            ...
            clean_image, clean_metadata = self.load_clean_image(
                clean_tile_id, clean_dir, device
            )
    ...
    def optimize_domain(self, domain: str, sensor: str = "default") -> Dict[str, Any]:
        ...
        print(f"\n{'='*80}")
        print(
            f"🔍 Optimizing Single-Domain Parameters: {domain.upper()} ({sensor.upper()})"
        )
        print(f"{'='*80}")
    ...
    def run_optimization(self):
        ...
        for domain in self.base_args.get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Single-domain parameter optimization for cross-domain low-light enhancement"
    )
    ...
    parser.add_argument(
        "--metadata_json", type=str, required=True, help="Path to metadata JSON file"
    )
    parser.add_argument(
        "--noisy_dir",
        type=str,
    ...
    # Optional arguments
    parser.add_argument(
        "--num_examples", type=int, default=20, help="Number of example images to test"
    )
    parser.add_argument(
        "--domains",
        type=str,
    ...
    parser.add_argument(
        "--output_base",
        type=str,
        default="results/single_domain_optimization",
