    ...
    def load_pg_results(pg_dir: Path) -> Dict[str, Dict[str, Any]]:
        ...
        if summary_path.exists():
            ...
            for result_info in summary.get("results", []):
                ...
                example_dirs = [
                    d for d in pg_dir.iterdir() if d.is_dir() and tile_id in d.name
                ]
                if example_dirs:
    ...
    def load_local_var_results(local_var_dir: Path) -> Dict[str, Dict[str, Any]]:
        ...
        if summary_path.exists():
            ...
            for result_info in summary.get("results", []):
                ...
                example_dirs = [
                    d
                    for d in local_var_dir.iterdir()
                    if d.is_dir() and tile_id in d.name
                ]
                if example_dirs:
                    example_dir = example_dirs[0]
    ...
    def load_normal_guidance_results(normal_dir: Path) -> Dict[str, Dict[str, Any]]:
        ...
        if summary_path.exists():
            ...
            for result_info in summary.get("results", []):
                ...
                # Find corresponding example directory
                example_dirs = [
                    d for d in normal_dir.iterdir() if d.is_dir() and tile_id in d.name
                ]
                if example_dirs:
                    example_dir = example_dirs[0]
...
def compute_metrics(
    ...
    if clean.shape != enhanced.shape:
        logger.warning(
            f"Shape mismatch: clean {clean.shape} vs enhanced {enhanced.shape}, skipping metrics"
        )
        return {
            "ssim": float("nan"),
            "psnr": float("nan"),
...
def load_noisy_image_from_results(
    tile_id: str, results_dir: Path
) -> Optional[torch.Tensor]:
    """Load noisy image from results directory."""
    # Find example directory containing this tile_id
    example_dirs = [
        d for d in results_dir.iterdir() if d.is_dir() and tile_id in d.name
    ]
    if example_dirs:
        example_dir = example_dirs[0]
...
def generate_synthetic_clean_reference(
    noisy_image: torch.Tensor, domain: str
) -> torch.Tensor:
    """
...
def create_comparison_visualization(
    ...
    if clean_image is not None:
        ...
        axes[panel_idx].imshow(
            clean_display, cmap="gray" if clean_display.ndim == 2 else None
        )
    ...
    for i, method_name in enumerate(method_names):
        ...
        (
            ...
            res_scale_min,
            res_scale_max,
        ) = prepare_for_display(result_phys)

        axes[panel_idx + i].imshow(
            result_display, cmap="gray" if result_display.ndim == 2 else None
        )
        ...
        if clean_image is not None:
            ...
            metrics = compute_metrics(
                clean_image.cpu().numpy(), result_tensor.cpu().numpy()
            )
    ...
    # Main title
    comparison_type = (
        "Low-Light Enhancement" if clean_image is None else "Synthetic Denoising"
    )
    plt.suptitle(
...
def main():
    ...
    # Output arguments
    parser.add_argument(
        "--output_dir", type=str, default="results/denoising_comparison"
    )
    parser.add_argument(
        "--domain",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of example images to compare",
    )
    ...
    local_var_results = ResultsLoader.load_local_var_results(
        Path(args.local_var_results_dir)
    )
    normal_results = ResultsLoader.load_normal_guidance_results(
        Path(args.normal_results_dir)
    )
    synthetic_results = ResultsLoader.load_synthetic_results(
        Path(args.synthetic_results_dir)
    )
    ...
    common_tile_ids = pg_tile_ids.intersection(local_var_tile_ids).intersection(
        normal_tile_ids
    )
    ...
    logger.info(
        f"Selected {len(selected_tile_ids)} tiles for comparison: {selected_tile_ids}"
    )
    ...
    for idx, tile_id in enumerate(selected_tile_ids):
        ...
        try:
            ...
            noisy_image = load_noisy_image_from_results(
                tile_id, Path(args.pg_results_dir)
            )
            ...
            if tile_id in pg_results:
                ...
                logger.info(
                    f"  ✓ Loaded PG guidance result: range [{pg_result.min():.3f}, {pg_result.max():.3f}]"
                )
            ...
            if tile_id in local_var_results:
                ...
                logger.info(
                    f"  ✓ Loaded local variance result: range [{local_var_result.min():.3f}, {local_var_result.max():.3f}]"
                )
            ...
            if tile_id in normal_results:
                ...
                logger.info(
                    f"  ✓ Loaded normal guidance result: range [{normal_result.min():.3f}, {normal_result.max():.3f}]"
                )
            ...
            if clean_image is not None:
                ...
                logger.info(
                    f"  ✓ Found corresponding clean reference: range [{clean_image.min():.3f}, {clean_image.max():.3f}]"
                )
            else:
                logger.warning(
                    f"  ⚠ No clean reference found for {tile_id}, skipping clean reference panel"
                )
            ...
            result_info = {
                ...
                "pg_metadata": pg_results[tile_id]["metadata"]
                if tile_id in pg_results
                else None,
                "local_var_metadata": local_var_results[tile_id]["metadata"]
                if tile_id in local_var_results
                else None,
            }
            ...
            logger.info(
                f"✓ Created fair comparison with same-scene pairs: {comparison_path}"
            )
