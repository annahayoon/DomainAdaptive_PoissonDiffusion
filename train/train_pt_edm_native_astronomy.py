#!/usr/bin/env python3
"""
Train restoration model using EDM's native training code with astronomy_v2/clean dataset.
...
- Uses astronomy_v2/clean dataset with metadata_astronomy_incremental.json
- Loads 32-bit float .pt files instead of 8-bit PNG
- NO quantization loss - preserves full precision
- Data is in [-1, 1] normalized range from pipeline's astronomy scaling:
  * Astronomy: (ADU * gain + 5.0) / 110.0 -> normalize to [-1, 1]
- Float32 throughout the pipeline
- 1 channel grayscale astronomy data
- Uses train/val splits as specified in metadata
"""
...
def main():
    """Main training function for astronomy_v2/clean dataset.

    The training script uses the astronomy_v2/clean dataset with:
    - Domain-specific physics calibration (ADU → electrons)
    - Astronomy-specific scaling: (electrons + 5.0) / 110.0
    - Final normalization to [-1, 1] using transforms.Normalize(mean=[0.5], std=[0.5])
    - Metadata JSON with train/val splits, calibration parameters, and scaling info
    - 1 channel grayscale astronomy data

    The training loop will:
    1. Load float32 .pt data already in [-1, 1] range from astronomy_v2/clean
    ...
    4. Use train/val splits as specified in metadata_astronomy_incremental.json
    """
    parser = argparse.ArgumentParser(
        description="Train model with EDM native training using astronomy_v2/clean dataset (NO QUANTIZATION)"
    )
