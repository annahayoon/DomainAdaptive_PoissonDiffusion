...
def process_domain_examples(domain, examples):
    ...
    for example_name in examples:
        ...
        # Process each method
        methods = ['noisy', 'clean', 'restored_exposure_scaled', 'restored_gaussian_x0', 'restored_pg_x0']
        method_ranges = {}
        ...
        for method in methods:
            ...
            if img_path.exists():
                ...
                scaled_tensor = scale_and_save_image(img_path, scale_factor, scaled_path)

                if scaled_tensor is not None:
                    img_range = get_image_range(scaled_tensor)
                    method_ranges[method] = img_range
                    print(f"  {method}: [{img_range['min']:.3f}, {img_range['max']:.3f}]")
