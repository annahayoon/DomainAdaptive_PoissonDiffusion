...
class PriorCleanTrainingManager:
    ...
    def validate_model(self, model: nn.Module, val_loader: DataLoader, device: torch.device, step: int) -> float:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                sigma = torch.exp(torch.randn(clean.shape[0], device=device) * 1.2 - 1.2)
    ...
    def save_phase_checkpoint(self, model: nn.Module, step: int, config: Dict[str, Any]):
        ...
        if phase_step is not None:
            phase_checkpoint_path = self.output_dir / f"phase_{phase_name}_step_{phase_step:06d}.pth"
            torch.save({
                'model_state_dict': model.state_dict(),
                'step': phase_step,
                'phase': phase_name,
        ...
        **config_kwargs
        ...
        if h100_optimizations and is_h100:
            ...
            config = {
                ...
                "max_steps": config_kwargs.get("max_steps", 450000),  # Reduced for faster completion
                "batch_size": config_kwargs.get("batch_size", 32),     # Large batch for H100
                "gradient_accumulation_steps": config_kwargs.get("gradient_accumulation_steps", 2),
                "learning_rate": config_kwargs.get("learning_rate", 1e-4),  # Higher for large batches
                ...
                "val_frequency": config_kwargs.get("val_frequency", 5000),  # Every 5K steps
                "save_frequency_steps": config_kwargs.get("save_frequency_steps", 5000),  # Every 5K steps
                "phase_save_frequency": config_kwargs.get("phase_save_frequency", 50000),  # Every 50K steps
...
def main():
    ...
    # Output arguments
    parser.add_argument("--output_dir", type=str, default="results/prior_clean_training")
    ...
    parser.add_argument("--phase_save_frequency", type=int, default=50000, help="Save phase checkpoints every N steps")
