        ...
        if domain_idx is not None:
            ...
            if domain == "astronomy":
                cross_domain_dir = Path(
                    "/home/jilab/Jae/results/cross_domain_inference_all_tiles/astronomy_cross_domain"
                )
            elif domain == "microscopy":
                cross_domain_dir = Path(
                    "/home/jilab/Jae/results/cross_domain_inference_all_tiles/microscopy_cross_domain"
                )
            elif domain in ["photography_sony", "photography_fuji"]:
                cross_domain_dir = Path(
                    "/home/jilab/Jae/results/cross_domain_inference_all_tiles/photography_cross_domain"
                )
...
def get_image_for_method(domain, example_dir, method):
    ...
    if "cross" in method:
        ...
        if not cross_example:
            print(
                f"Warning: No cross-domain example found for {domain}/{tile_path.name}"
            )
            return None
        ...
        if domain == "astronomy":
            source_path = (
                Path(
                    "/home/jilab/Jae/results/optimized_inference_all_tiles/astronomy_optimized"
                )
                / tile_path.name
            )
        elif domain == "microscopy":
            source_path = (
                Path(
                    "/home/jilab/Jae/results/optimized_inference_all_tiles/microscopy_optimized"
                )
                / tile_path.name
            )
            ...
            source_path = (
                Path(
                    "/home/jilab/Jae/results/optimized_inference_all_tiles/photography_sony_optimized"
                )
                / tile_path.name
            )
...
def create_domain_subplot(
    ax, domain, example_dir, methods, metrics_dict=None, scale_mode="pg_x0_cross"
):
    ...
    for method in methods:
        ...
        if img is not None:
            ...
            print(
                f"  {method}: range [{pixel_ranges[-1]['min']:.2f}, {pixel_ranges[-1]['max']:.2f}], RGB={is_rgb}"
            )
        else:
...
def extract_metrics(domain, example_dir, method):
    ...
    if "cross" in method:
        ...
        if not cross_example:
            print(
                f"Warning: No cross-domain example found for {domain}/{Path(example_dir).name}"
            )
            return None
...
def main():
    ...
    fig_height = (
        len(domains) * 3.5
    )  # 3.5 inches per domain (reduced for tighter layout)
    ...
    for i, domain in enumerate(domains):
        ...
        pixel_ranges, images, vmin, vmax = create_domain_subplot(
            axes[i],
            domain,
            example_path,
            methods,
            metrics_dict=metrics_dict,
            scale_mode="pg_x0_cross",
        )
        ...
        axes[i].set_title(
            f"{domain_display_name}", fontsize=12, fontweight="bold", y=1.02
        )

    # Overall title
    fig.suptitle(
        "Low-light imaging restoration comparison", fontsize=14, fontweight="bold"
    )
    ...
    plt.savefig(
        "comparison_visualization_domain_scaled_high_res.png",
        dpi=300,
        bbox_inches="tight",
    )
    print(
        "High resolution version saved to comparison_visualization_domain_scaled_high_res.png"
    )

    # Create version scaled to clean range
    print("\nCreating clean-range scaled version...")
    fig_clean, axes_clean = plt.subplots(
        len(domains), 1, figsize=(fig_width, fig_height)
    )
    if len(domains) == 1:
    ...
    for i, domain in enumerate(domains):
        ...
        axes_clean[i].set_title(
            f"{domain_display_name}", fontsize=12, fontweight="bold", y=1.02
        )

    # Overall title
    fig_clean.suptitle(
        "Low-light imaging restoration comparison\n(Clean Reference Scaled)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    plt.savefig(
        "comparison_visualization_domain_scaled_clean_range.png",
        dpi=150,
        bbox_inches="tight",
    )
    print(
        "Clean-range version saved to comparison_visualization_domain_scaled_clean_range.png"
    )

    plt.savefig(
        "comparison_visualization_domain_scaled_clean_range_high_res.png",
        dpi=300,
        bbox_inches="tight",
    )
