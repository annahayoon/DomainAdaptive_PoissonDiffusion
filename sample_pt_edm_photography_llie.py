#!/usr/bin/env python3
"""
EDM Photography LLIE (Low-Light Image Enhancement) with Step-by-Step Visualizations

This script demonstrates the complete EDM-based low-light image enhancement process starting from real noisy test images,
showing detailed step-by-step visualizations of the enhancement process.

Key Features:
- Starts from real noisy photography tiles (not pure noise)
- Shows complete low-light enhancement process with step-by-step visualization
...
- Demonstrates physics-aware processing in physical units
...
class ComprehensiveEDMLLIE:
    """
    Comprehensive EDM low-light image enhancement with step-by-step visualization capabilities.

    This enhancement process starts from real noisy test images and shows the
    complete low-light enhancement process with detailed visualizations.
    """
    ...
    def __init__(
        ...
        """Initialize the restoration system with trained model."""
    ...
    def enhance_with_visualization(
        ...
        """
        Perform EDM low-light image enhancement starting from a real noisy image.

        Args:
            noisy_image: Real noisy image tensor in [-1, 1] range
        ...
        logger.info(
            f"✓ Starting from real noisy image: shape {noisy_image.shape}, range [{noisy_image.min():.3f}, {noisy_image.max():.3f}]"
        )
        ...
        # Main enhancement loop - START FROM NOISY IMAGE, NOT PURE NOISE!
        # IMPORTANT: Must scale initial noisy image by initial sigma for proper EDM sampling
        x_next = noisy_image.to(torch.float64) * t_steps[0]  # Scale by initial sigma
    ...
    def normalize_tensor_for_display(
        ...
        if tensor_data.ndim == 3:
            if domain == "photography" and tensor_data.shape[0] == 3:  # RGB
                # For RGB, convert to grayscale for display (like original pipeline)
    ...
    def create_comprehensive_comparison(
        ...
        # Row 1: Model space [-1, 1] - Noisy Input
        self._add_panel(axes[0, 0], initial_noisy, "Noisy Input", "[-1, 1] Model Space")

        # Row 1: Intermediate steps in model space
        ...
        # Row 1: Final restored in model space
        self._add_panel(
            axes[0, -1], final_restored, "Final Restored", "[-1, 1] Model Space"
        )
        ...
        axes[1, 0].set_ylabel(
            "Physical Units\n[domain_min, domain_max]", fontsize=12, fontweight="bold"
        )
        axes[2, 0].set_ylabel(
        ...
        plt.suptitle(
            ...
            + f"Starting from real noisy image → Progressive enhancement → Final enhanced image",
            fontsize=14,
            fontweight="bold",
        )
        plt.tight_layout()
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("COMPREHENSIVE EDM LLIE WITH STEP-BY-STEP VISUALIZATIONS")
    logger.info("=" * 80)
    ...
    for sample_idx in range(args.num_samples):
        ...
        try:
            ...
            if img_model.ndim == 3 and img_model.shape[2] == 3:
                ...
                img_model_norm = np.zeros_like(img_model)
                for c in range(3):
                    channel = img_model[:, :, c]
                    valid_mask = np.isfinite(channel)
                    if np.any(valid_mask):
                        ...
                        img_model_norm[:, :, c] = (clipped - p_low) / (
                            p_high - p_low + 1e-8
                        )
                    else:
                        img_model_norm[:, :, c] = 0.5
    ...
    logger.info("=" * 80)
    logger.info("🎉 COMPREHENSIVE LLIE COMPLETED!")
    logger.info(f"📁 All results saved to: {output_dir}")
    logger.info(
        f"📊 Summary: {args.num_samples} enhancements, {args.num_steps} steps each"
    )
    logger.info(f"🎯 Starting from real noisy images → enhanced/cleaned images")
    logger.info("=" * 80)
