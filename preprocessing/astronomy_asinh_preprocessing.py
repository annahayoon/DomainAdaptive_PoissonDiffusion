...
class AstronomyAsinhPreprocessor:
    ...
    def preprocess_astronomy_image(
        self,
        image: np.ndarray,
        calibration: Dict[str, Any],
        apply_cosmic_ray_removal: bool = True,
    ) -> Dict[str, Any]:
        """
        ...
        if apply_cosmic_ray_removal:
            cosmic_ray_mask = self._detect_cosmic_rays(
                image_bg_subtracted, read_noise=calibration.get("read_noise", 4.72)
            )
        ...
        image_asinh = self._apply_asinh_scaling(
            image_clean, scale=self._global_scale, softening=self._softening_factor
        )
        ...
        return {
            ...
            "preprocessing_metadata": {
                ...
                "cosmic_rays_detected": cosmic_ray_mask.sum()
    ...
    def _estimate_background(self, image: np.ndarray) -> float:
        ...
        if len(np.unique(flattened)) <= 2:
            ...
            self.logger.debug(
                f"Background estimated (simple): {background:.2f} electrons"
            )
    ...
    def _detect_cosmic_rays_2d(
        self, image_2d: np.ndarray, read_noise: float
    ) -> np.ndarray:
    ...
    def _remove_cosmic_rays(
        self, image: np.ndarray, cosmic_ray_mask: np.ndarray
    ) -> np.ndarray:
            ...
            if image.ndim > 2:
                ...
                for i in range(image.shape[0]):
                    if cosmic_ray_mask[i].sum() > 0:
                        ...
                        image_clean[i][cosmic_ray_mask[i]] = median_filtered[
                            cosmic_ray_mask[i]
                        ]
            else:
                # Single 2D image
    ...
    def _apply_asinh_scaling(
        self, image: np.ndarray, scale: float, softening: float
    ) -> np.ndarray:
        """
        ...
        self.logger.debug(
            f"Asinh scaling applied: softening={softening:.3f}, scale={scale:.2f}"
        )
...
def main():
    ...
    # Background + noise
    image = np.random.poisson(background_level, (image_size, image_size)).astype(
        np.float32
    )
    ...
    for _ in range(n_sources):
        ...
        brightness = np.random.exponential(
            1000
        )  # Exponential distribution of brightnesses
    ...
    config = AsinhScalingConfig(
        softening_factor=1e-3, scale_percentile=99.5, background_percentile=10.0
    )

    preprocessor = AstronomyAsinhPreprocessor(config)

    calibration = {"gain": 4.62, "read_noise": 4.72}
