...
class PhysicsBasedCalibrationMethod(ABC):
    ...
    @abstractmethod
    def estimate_calibration(self, images: List[np.ndarray], 
                           metadata: List[Dict[str, Any]]) -> PhysicsBasedCalibrationData:
        """Estimate calibration parameters using domain-specific physics"""
        pass
    
    @abstractmethod
    def validate_calibration(self, calibration: PhysicsBasedCalibrationData,
                           images: List[np.ndarray]) -> Dict[str, float]:
...
class PhotographyPhotonTransferCalibration(PhysicsBasedCalibrationMethod):
    ...
    def __init__(self):
        ...
        self.camera_physics = {
            "sony": {
                ...
                "expected_gain_range": (0.79, 2.1),  # e-/ADU (measured: 0.79 at ISO 4000, 2.1 at ISO 2000)
                "expected_read_noise_range": (2.0, 6.0),  # electrons (measured: 2-3e above ISO 4000, 6e below)
                "bit_depth": 14,
                "iso_specific_gains": {"iso_2000": 2.1, "iso_4000": 0.79},  # Measured values
                "iso_specific_read_noise": {"high_iso": 2.5, "low_iso": 6.0}  # Measured ranges
            },
            "fuji": {
                ...
                "expected_read_noise_range": (2.5, 2.5),  # electrons (measured at base ISO)
                "bit_depth": 16,
    ...
    def estimate_calibration(self, image_pairs: List[Tuple[np.ndarray, np.ndarray]], 
        ...
        self.logger.info(f"Estimating calibration for {camera_type} camera using photon transfer")
        
        if len(image_pairs) < 2:
            self.logger.warning("Insufficient image pairs for robust photon transfer curve")
            return self._fallback_calibration(camera_type, physics, image_pairs, metadata)
        ...
        gain, read_noise, fit_quality = self._fit_photon_transfer_curve(signals, variances, weights)
    ...
    def _compute_photon_transfer_data(self, image_pairs: List[Tuple[np.ndarray, np.ndarray]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for short_img, long_img in image_pairs:
            ...
            if long_mean > short_mean * 1.5:  # Valid pair
                ...
                variances.append(diff_var / 2.0)  # Divide by 2 for single image variance
                weights.append(weight)
        
        return np.array(signals), np.array(variances), np.array(weights)
    
    def _fit_photon_transfer_curve(self, signals: np.ndarray, variances: np.ndarray, 
    ...
    def _validate_against_physics(self, gain: float, read_noise: float, 
                                 physics: Dict[str, Any]) -> Tuple[float, float]:
        ...
        if gain < gain_min or gain > gain_max:
            self.logger.warning(f"Gain {gain:.2f} outside expected range [{gain_min}, {gain_max}]")
            gain = np.clip(gain, gain_min, gain_max)
        
        if read_noise < rn_min or read_noise > rn_max:
            self.logger.warning(f"Read noise {read_noise:.2f} outside expected range [{rn_min}, {rn_max}]")
            read_noise = np.clip(read_noise, rn_min, rn_max)
        
        return gain, read_noise
    
    def _estimate_uncertainties(self, signals: np.ndarray, variances: np.ndarray,
                               weights: np.ndarray, gain: float, read_noise: float) -> Tuple[float, float]:
    ...
    def _fallback_calibration(self, camera_type: str, physics: Dict[str, Any],
    ...
    def validate_calibration(self, calibration: PhysicsBasedCalibrationData,
        ...
        gain_min, gain_max = physics["expected_gain_range"]
        validation["gain_in_range"] = 1.0 if gain_min <= calibration.gain <= gain_max else 0.0
        
        rn_min, rn_max = physics["expected_read_noise_range"]
        validation["read_noise_in_range"] = 1.0 if rn_min <= calibration.read_noise <= rn_max else 0.0
...
class MicroscopyPhotonLimitedCalibration(PhysicsBasedCalibrationMethod):
    ...
    def __init__(self):
        ...
        self.scmos_physics = {
            ...
            "expected_read_noise_range": (1.0, 2.0),  # electrons RMS per pixel (BioSR measured range)
            "bit_depth": 16,
    ...
    def _estimate_from_noise_levels(self, images: List[np.ndarray], 
                                   noise_levels: List[int]) -> Tuple[float, float]:
    ...
    def _validate_scmos_physics(self, gain: float, read_noise: float) -> Tuple[float, float]:
        """Validate against sCMOS physics"""
        ...
        if gain < gain_min or gain > gain_max:
            self.logger.warning(f"Gain {gain:.2f} outside sCMOS range [{gain_min}, {gain_max}]")
            gain = np.clip(gain, gain_min, gain_max)
        
        if read_noise < rn_min or read_noise > rn_max:
            self.logger.warning(f"Read noise {read_noise:.2f} outside sCMOS range [{rn_min}, {rn_max}]")
            read_noise = np.clip(read_noise, rn_min, rn_max)
    ...
    def _compute_microscopy_confidence(self, images: List[np.ndarray], 
                                      noise_levels: List[int]) -> float:
...
class AstronomyHeaderBasedCalibration(PhysicsBasedCalibrationMethod):
    ...
    def __init__(self):
        ...
        self.hst_physics = {
            "ACS": {
                ...
                "expected_gain_range": (1.0, 1.0),  # e-/DN (measured: ~1.0 for most amplifiers)
                "expected_read_noise_range": (2.0, 5.0),  # electrons (measured: 2.0-5.0 across amplifiers)
                "bit_depth": 16,
            ...
            "WFC3_UVIS": {
                ...
                "expected_gain_range": (1.0, 4.0),  # e-/DN (measured supported: 1, 1.5, 2, 4)
                "expected_read_noise_range": (2.95, 3.22),  # electrons (measured mean across amplifiers)
                "bit_depth": 16,
            ...
            "WFC3_IR": {
                ...
                "expected_gain_range": (2.0, 4.0),  # e-/DN (measured supported: 2.0, 2.5, 3.0, 4.0)
                "expected_read_noise_range": (10.0, 20.0),  # electrons (measured: higher than UVIS, <20)
                "bit_depth": 16,
            ...
            "WFPC2": {
                ...
                "quantum_efficiency": 0.6,  # Peak QE
                "expected_gain_range": (7.0, 14.0),  # e-/DN (measured ATD-GAIN=7 and 14)
                "expected_read_noise_range": (5.0, 8.0),  # electrons RMS (measured range at gain 7)
                "bit_depth": 12,
    ...
    def estimate_calibration(self, images: List[np.ndarray], 
        ...
        if gains:
            ...
            gain = (physics["expected_gain_range"][0] + physics["expected_gain_range"][1]) / 2
            gain_uncertainty = gain * 0.2
        
        if read_noises:
            read_noise = float(np.median(read_noises))
            read_noise_uncertainty = float(np.std(read_noises)) if len(read_noises) > 1 else read_noise * 0.1
        else:
            # Fallback to typical values
            read_noise = (physics["expected_read_noise_range"][0] + physics["expected_read_noise_range"][1]) / 2
    ...
    def _validate_hst_physics(self, gain: float, read_noise: float, 
                             physics: Dict[str, Any]) -> Tuple[float, float]:
        ...
        if gain < gain_min or gain > gain_max:
            self.logger.warning(f"Gain {gain:.2f} outside {physics} range [{gain_min}, {gain_max}]")
            gain = np.clip(gain, gain_min, gain_max)
        
        if read_noise < rn_min or read_noise > rn_max:
            self.logger.warning(f"Read noise {read_noise:.2f} outside {physics} range [{rn_min}, {rn_max}]")
            read_noise = np.clip(read_noise, rn_min, rn_max)
        
        return gain, read_noise
    
    def _validate_with_images(self, images: List[np.ndarray], 
                             gain: float, read_noise: float) -> bool:
    ...
    def _compute_astronomy_confidence(self, n_gains: int, n_read_noises: int, 
    ...
    def validate_calibration(self, calibration: PhysicsBasedCalibrationData,
        ...
        if images:
            validation["image_consistency"] = 1.0 if self._validate_with_images(
