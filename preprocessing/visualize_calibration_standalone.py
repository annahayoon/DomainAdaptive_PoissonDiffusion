...
def apply_photography_calibration(
    rggb_image: np.ndarray, gain: float, read_noise: float, metadata: dict
):
    """
    ...
    if domain == "astronomy":
        ...
        try:
            ...
            result = astro_preprocessor.preprocess_astronomy_image(
                image, calibration_dict, apply_cosmic_ray_removal=True
            )
            ...
            logger.info(
                f"      Background: {preprocessing_metadata['background_level']:.2f} e-"
            )
            logger.info(
                f"      Scale factor: {preprocessing_metadata['scale_factor']:.2f} e-"
            )
            logger.info(
                f"      Cosmic rays detected: {preprocessing_metadata['cosmic_rays_detected']}"
            )
            logger.info(
                f"      Softening factor: {preprocessing_metadata['softening_factor']:.3f}"
            )
            ...
            if len(image_flat) > 0:
                ...
                scaled = scaled * 2.0
                logger.info(
                    f"   📊 Fallback astronomical scaling: softening={softening:.6f}"
                )
            else:
                scaled = image
                logger.warning(
        ...
        logger.info(
            f"   📊 Linear scaling for {domain}: range=[{img_min:.3f}, {img_max:.3f}]"
        )
...
def visualize_calibration_effects():
    ...
    test_files = {
        ...
        "microscopy": {
            "noisy": str(
                base_path
                / "raw/microscopy/structures/Microtubules/Cell_005/RawSIMData_level_02.mrc"
            ),
            "clean": str(
                base_path
                / "raw/microscopy/structures/Microtubules/Cell_005/RawSIMData_gt.mrc"
            ),
        },
        "astronomy": {
            "clean": str(
                base_path
                / "raw/astronomy/hla_associations/direct_images/j8hp9fq1q_detection_sci.fits"
            ),  # Direct image = clean reference
            "noisy": str(
                base_path
                / "raw/astronomy/hla_associations/g800l_images/j8hp9fq1q_g800l_sci.fits"
            ),  # G800L = noisy (spectroscopic with artifacts)
        },
    }
    ...
    for domain_name, file_pair in test_files.items():
        ...
        try:
            ...
            gain, read_noise, calibration_method = get_physics_calibration(
            ...
            if domain_name == "photography":
                ...
                noisy_tile_calibrated = apply_physics_calibration(
                    noisy_tile_raw, gain, read_noise
                )
                logger.info(
                    f"   📷 Applied physics calibration (gain={gain:.1f}, read_noise={read_noise:.1f})"
                )
            ...
            # Apply domain-specific scaling (optimized for astronomical data)
            noisy_tile_scaled = apply_astronomical_scaling(
                noisy_tile_calibrated, domain_name
            )

            # Normalize and convert to 8-bit
            noisy_min, noisy_max = float(np.min(noisy_tile_scaled)), float(
                np.max(noisy_tile_scaled)
            )
                ...
                if domain_name == "photography":
                    ...
                    rgb_tile = np.stack(
                        [
                            ...
                            (noisy_tile_raw_norm[1] + noisy_tile_raw_norm[2])
                            / 2,  # Average G channels
                ...
                if domain_name == "photography":
                    ...
                    rgb_tile = np.stack(
                        [
                            ...
                            (clean_tile_raw_norm[1] + clean_tile_raw_norm[2])
                            / 2,  # Average G channels
                ...
                if domain_name == "photography":
                    ...
                    rgb_tile = np.stack(
                        [
                            ...
                            (
                                noisy_tile_calibrated_norm[1]
                                + noisy_tile_calibrated_norm[2]
                            )
                            / 2,  # Average G channels
            ...
            if domain_name == "astronomy":
                axes[2].set_title(
                    f"Physics-Calibrated + Astronomical Scaling\n(8-bit PNG, asinh optimized for low signal)",
                    fontsize=12,
                    fontweight="bold",
                )
            elif domain_name == "photography":
                axes[2].set_title(
                    f"Physics-Calibrated RGGB\n(8-bit PNG, gain={gain:.1f}, read_noise={read_noise:.1f})",
                    fontsize=12,
                    fontweight="bold",
                )
            else:
                axes[2].set_title(
                    f"Physics-Calibrated + Linear Scaled\n(8-bit PNG, {calibration_method})",
                    fontsize=12,
                    fontweight="bold",
                )
            axes[2].axis("off")
