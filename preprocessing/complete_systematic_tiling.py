...
class SystematicTilingConfig:
    ...
    def __post_init__(self):
        ...
        if self.stride <= 0:
            raise ValueError(
                f"Stride must be positive. Got stride={self.stride} "
...
class TileInfo:
    ...
    valid_region: Tuple[
        int, int, int, int
    ]  # (y_start, y_end, x_start, x_end) of valid data
...
class SystematicTiler:
    ...
    def _validate_config(self):
        ...
        if not 0 <= self.config.overlap_ratio < 0.5:
            raise ValueError(
                f"Overlap ratio must be in [0, 0.5), got {self.config.overlap_ratio}"
            )

        if self.config.coverage_mode not in [
            "complete",
            "valid_only",
            "sliding_window",
        ]:
    ...
    def extract_tiles(self, image: np.ndarray) -> List[TileInfo]:
        ...
        self.logger.info(
            f"Extracting tiles from {H}×{W} image with {self.config.tile_size}×{self.config.tile_size} tiles"
        )
        self.logger.info(
            f"Configuration: overlap={self.config.overlap_ratio:.2f}, stride={self.config.stride}, mode={self.config.coverage_mode}"
        )
        ...
        self.logger.info(
            f"Extracted {len(tiles)} tiles with {self.config.coverage_mode} coverage"
        )

        # Compute coverage statistics
        coverage_stats = self._compute_coverage_stats(tiles, (H, W))
        self.logger.info(
            f"Coverage: {coverage_stats['coverage_ratio']:.1%}, "
    ...
    def _extract_complete_coverage(self, image: np.ndarray) -> List[TileInfo]:
        ...
        for i in range(tiles_h):
            for j in range(tiles_w):
                ...
                valid_y_end = (
                    min(self.config.tile_size, H - orig_y)
                    if orig_y >= 0
                    else min(self.config.tile_size, H + abs(orig_y))
                )
                valid_x_start = max(0, -orig_x) if orig_x < 0 else 0
                valid_x_end = (
                    min(self.config.tile_size, W - orig_x)
                    if orig_x >= 0
                    else min(self.config.tile_size, W + abs(orig_x))
                )

                # Ensure valid bounds
                valid_y_start = max(0, min(valid_y_start, self.config.tile_size))
                valid_y_end = max(
                    valid_y_start, min(valid_y_end, self.config.tile_size)
                )
                valid_x_start = max(0, min(valid_x_start, self.config.tile_size))
                valid_x_end = max(
                    valid_x_start, min(valid_x_end, self.config.tile_size)
                )

                # Calculate valid ratio
                valid_pixels = (valid_y_end - valid_y_start) * (
                    valid_x_end - valid_x_start
                )
                ...
                tile_info = TileInfo(
                    ...
                    padding_applied={"pad_h": pad_h, "pad_w": pad_w}
                    if (pad_h > 0 or pad_w > 0)
                    else None,
    ...
    def _compute_coverage_stats(
        self, tiles: List[TileInfo], image_shape: Tuple[int, int]
    ) -> Dict[str, float]:
        """Compute coverage statistics"""
        ...
        return {
            ...
            "avg_coverage": np.mean(coverage_map[coverage_map > 0])
            if covered_pixels > 0
            else 0,
...
def get_domain_tiling_config(
    domain_name: str, tile_size: int = None
) -> SystematicTilingConfig:
