...
class CalibrationData:
    ...
    norm_percentiles: Tuple[float, float] = (
        0.1,
        99.9,
    )  # Percentiles used for robust normalization
...
class BaseDomainProcessor(ABC):
    ...
    def compute_normalization_parameters(
        self, images: List[np.ndarray], calibration: CalibrationData
    ) -> CalibrationData:
        ...
        for image in images:
            ...
            if hasattr(calibration, "background"):
                image_electrons = np.maximum(
                    image_electrons - calibration.background, 0
                )
    ...
    def preprocess_image(
        ...
        if calibration.norm_min is not None and calibration.norm_max is not None:
            ...
            image_clipped = np.clip(
                image_electrons, calibration.norm_min, calibration.norm_max
            )
            # Normalize to [0,1]
            if calibration.norm_max > calibration.norm_min:
                normalized = (image_clipped - calibration.norm_min) / (
                    calibration.norm_max - calibration.norm_min
                )
            else:
    ...
    def extract_tiles_unified(self, image: np.ndarray) -> List[Dict[str, Any]]:
        ...
        for tile_info in tiles:
            ...
            if tile_info.tile_data.shape[-2:] != (256, 256):
                self.logger.warning(
                    f"Tile {tile_info.tile_id} is not 256x256: {tile_info.tile_data.shape}"
                )
                continue
    ...
    def extract_tiles_systematic(
        self, image: np.ndarray, tile_size: int = None
    ) -> List[Dict[str, Any]]:
        """Legacy method - now uses unified 256x256 resolution"""
        return self.extract_tiles_unified(image)

    def extract_tiles(
        self, image: np.ndarray, tile_size: int = None, overlap: float = None
    ...
    def compute_image_statistics(
        self, image: np.ndarray, valid_mask: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
...
class PhotographyProcessor(BaseDomainProcessor):
    ...
    def discover_files(self, raw_data_path: str) -> List[Dict[str, Any]]:
        ...
        self.logger.info(
            f"Total discovered {len(pairs)} photography pairs (Sony + Fuji)"
        )
        return pairs

    def _discover_camera_pairs(
        self, short_path: Path, long_path: Path, camera_type: str, pattern: str
    ) -> List[Dict[str, Any]]:
                    ...
                    pairs.append(
                        {
                            ...
                            "exposure_short": self._extract_exposure_time(
                                short_file.stem
                            ),
                            "exposure_long": self._extract_exposure_time(
                                long_candidates[0].stem
                            ),
                            "file_format": self.camera_defaults[camera_type][
                                "file_extension"
                            ],
                        }
                    )
    ...
    def load_image(self, file_path: str) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        with rawpy.imread(file_path) as raw:
            ...
            camera_defaults = self.camera_defaults.get(
                camera_type, self.camera_defaults["sony"]
            )
        ...
        packed = self._pack_bayer_to_4channel(
            bayer, metadata["black_level"], camera_type
        )

        return packed, metadata

    def _pack_bayer_to_4channel(
        self, bayer: np.ndarray, black_level: np.ndarray, camera_type: str = "sony"
    ) -> np.ndarray:
    ...
    def estimate_calibration(
        ...
        if image_pairs and isinstance(image_pairs[0], np.ndarray):
            ...
            image_pairs = [
                (img, img) for img in single_images[: len(single_images) // 2]
            ]
        ...
        self.logger.info(
            f"Physics-based calibration: gain={physics_calib.gain:.3f}±{physics_calib.gain_uncertainty:.3f}, "
        ...
        return {
            "photography_meta": {
                ...
                "file_format": file_info.get(
                    "file_format", camera_defaults["file_extension"]
                ),
...
class MicroscopyProcessor(BaseDomainProcessor):
    ...
    def discover_files(self, raw_data_path: str) -> List[Dict[str, Any]]:
        ...
        for structure in self.biosr_config["structures"]:
            ...
            cell_dirs = [
                d
                for d in structure_path.iterdir()
    ...
    def estimate_calibration(self, images: List[np.ndarray]) -> CalibrationData:
        ...
        calibration = CalibrationData(
            ...
            read_noise=self.scmos_defaults[
                "read_noise"
            ],  # sCMOS read noise: 1-2 electrons
        ...
        return {
            "microscopy_meta": {
                ...
                "camera_specifications": {
                    ...
                    "dark_current_e_per_pixel_per_s": self.scmos_defaults[
                        "dark_current"
                    ],
                },
            }
