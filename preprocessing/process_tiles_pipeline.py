...
try:
    ...
    logger.warning("⚠️ BioSR MRC reader not available - microscopy MRC files may not load properly")
...
class SimpleTilesPipeline:
        ...
        def prepare_tile_data(self, tile_data: np.ndarray) -> Tuple[bytes, Dict[str, Any]]:
            ...
            try:
                ...
                buffer = io.BytesIO()
                pil_image.save(buffer, format='PNG', compress_level=6)  # 6 is balanced compression
                ...
                storage_info = {
                    ...
                    "compression_ratio": tile_data.nbytes / len(png_bytes) if len(png_bytes) > 0 else 1.0,
        ...
        def demosaic_rggb_to_rgb(self, rggb_image: np.ndarray, gains: np.ndarray = None, camera_type: str = "generic"):
            """
        ...
        def save_tile_as_png(self, tile_data: np.ndarray, tile_id: str, domain: str, data_type: str, 
                           gain: float, read_noise: float, calibration_method: str) -> Dict[str, Any]:
            ...
            try:
                ...
                if domain == "photography" and tile_uint8.shape[0] == 3:
                    ...
                    rgb_tile_uint8 = np.clip(rgb_tile_float * 255.0, 0, 255).astype(np.uint8)
        ...
        def process_file_to_png_tiles(self, file_path: str, domain: str) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if domain == "photography":
                    ...
                    if downsample_factor != 1.0:
                        original_shape = image.shape
                        image = self._downsample_with_antialiasing(image, downsample_factor)
                        logger.info(f"🔬 Microscopy downsampled: {original_shape} → {image.shape} (factor: {downsample_factor:.2f}x)")
                    else:
                        logger.info(f"🔬 Microscopy: no downsampling needed (factor: {downsample_factor:.2f}x)")
                    ...
                    if current_max_dim > target_max_dim:
                        ...
                        image = self._downsample_with_antialiasing(image, downsample_factor)
                        new_H, new_W = image.shape[1], image.shape[2] if len(image.shape) == 3 else image.shape[0], image.shape[1]
                        logger.info(f"🌟 Astronomy downsampled: {original_shape} → {image.shape} (factor: {downsample_factor:.2f}x)")
                        logger.info(f"   Aspect ratio preserved: {W/H:.3f} → {new_W/new_H:.3f}")
                    else:
                        logger.info(f"🌟 Astronomy image already at target size or smaller: {image.shape}")
                
                # === STEP 2: Apply domain-specific calibration to FULL image ===
                gain, read_noise, calibration_method = self._get_physics_based_calibration(domain, metadata)
                ...
                if domain == "photography":
                    ...
                    calibrated_image = self._apply_physics_calibration(image, gain, read_noise, domain)
                ...
                if domain == "photography":
                    ...
                    img_min, img_max = float(np.min(calibrated_image)), float(np.max(calibrated_image))
                    if img_max > img_min:
                    ...
                    try:
                        ...
                        if len(image_flat) > 0:
                            ...
                            scaled_image = np.arcsinh(calibrated_image / (softening + 1e-10))
                    ...
                    if img_mean < img_max * 0.01 and img_max > 0:
                        logger.warning(f"Very dark image detected, using percentile normalization")
                        ...
                        if p99 > p1:
                            ...
                            scaled_image = np.clip(calibrated_image / (img_max + 1e-8), 0, 1)
                ...
                if img_max > img_min and np.isfinite(img_min) and np.isfinite(img_max):
                    ...
                    logger.warning(f"Invalid range after scaling: [{img_min}, {img_max}], using fallback")
                ...
                for i in range(max_tiles):
                    ...
                    try:
                        ...
                        tile_metadata = self.save_tile_as_png(
                            tile_to_save, tile_id, domain, data_type, 
                            gain, read_noise, calibration_method
                        )
                    ...
                    "image_y": int(tile_info.image_position[1]),
                                ...
                                "quality_score": float(np.mean(tile_float)) if np.isfinite(np.mean(tile_float)) else 0.0,
                    "valid_ratio": float(tile_info.valid_ratio),
                    "is_edge_tile": bool(tile_info.is_edge_tile),
                    "overlap_ratio": float(self.overlap_ratios.get(domain, 0.1)),
                    ...
                    "electron_std": float(electron_std),
                                "norm_min": float(np.min(tile_float)) if np.isfinite(np.min(tile_float)) else 0.0,
                                "norm_max": float(np.max(tile_float)) if np.isfinite(np.max(tile_float)) else 1.0,
                                "norm_mean": float(np.mean(tile_float)) if np.isfinite(np.mean(tile_float)) else 0.5,
                                "norm_std": float(np.std(tile_float)) if np.isfinite(np.std(tile_float)) else 0.1,
                                "split": split,
                                "scene_id": scene_id
                            })
                ...
                logger.info(f"Generated {len(processed_tiles)} tiles from {Path(file_path).name}")
        ...
        def run_png_tiles_pipeline(self, max_files_per_domain: int = None):
            ...
            sony_files = list(Path("/home/jilab/anna_OS_ML/PKL-DiffusionDenoising/data/raw/SID/Sony").rglob("*.ARW"))
            fuji_files = list(Path("/home/jilab/anna_OS_ML/PKL-DiffusionDenoising/data/raw/SID/Fuji").rglob("*.RAF"))
            ...
            microscopy_files = list(Path("/home/jilab/anna_OS_ML/PKL-DiffusionDenoising/data/raw/microscopy").rglob("*.mrc"))
            ...
            astronomy_files = list(Path("/home/jilab/anna_OS_ML/PKL-DiffusionDenoising/data/raw/astronomy").rglob("*.fits"))
            ...
            metadata_path = self.base_path / "processed" / "comprehensive_tiles_metadata.json"
            ...
            logger.info(f"PNG Tiles Pipeline Completed: {results['total_tiles']:,} tiles generated")
        ...
        def _get_physics_based_calibration(self, domain: str, metadata: Dict[str, Any] = None) -> Tuple[float, float, str]:
        ...
        def _apply_physics_calibration(self, image: np.ndarray, gain: float, read_noise: float, domain: str) -> np.ndarray:
            """
        ...
        def _extract_sony_tiles(self, image: np.ndarray) -> List[Any]:
            ...
            try:
                ...
                logger.info(f"   ✅ Sony: {len(tiles)} tiles (256×256, overlap: {overlap_h}×{overlap_w}px)")
                return tiles
        ...
        def _extract_fuji_tiles(self, image: np.ndarray) -> List[Any]:
            ...
            try:
                ...
                overlap_h = tile_size - stride_h
                overlap_w = tile_size - stride_w
                logger.info(f"   ✅ Fuji: {len(tiles)} tiles (256×256, overlap: {overlap_h}×{overlap_w}px)")
                return tiles
        ...
        def _extract_astronomy_tiles(self, image: np.ndarray) -> List[Any]:
            ...
            try:
                ...
                overlap_h = tile_size - stride_h
                overlap_w = tile_size - stride_w
                logger.info(f"   ✅ Astronomy: {len(tiles)} tiles (256×256, overlap: {overlap_h}×{overlap_w}px)")
                return tiles
        ...
        def _extract_microscopy_tiles(self, image: np.ndarray) -> List[Any]:
            ...
            try:
                ...
                overlap_h = tile_size - stride_h
                overlap_w = tile_size - stride_w
                logger.info(f"   ✅ Microscopy: {len(tiles)} tiles (256×256, overlap: {overlap_h}×{overlap_w}px)")
                return tiles
        ...
        def _downsample_with_antialiasing(self, image: np.ndarray, factor: float) -> np.ndarray:
            """
            ...
            try:
                ...
                if len(image.shape) == 3:  # CHW format
                    ...
                    downsampled = zoom(filtered_image, zoom_factors, order=1)  # Linear interpolation
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Simple PNG Tiles Pipeline with Domain-Specific Calibration")
    ...
    if results.get("total_tiles", 0) > 0:
        ...
        for domain, stats in results.get('domains', {}).items():
            print(f"   • {domain.upper()}: {stats['tiles_generated']} tiles from {stats['files_processed']} files")
...
def get_physics_based_calibration(domain: str, metadata: Dict[str, Any] = None) -> Tuple[float, float, str]:
    """Get domain-specific physics-based calibration parameters"""
    ...
    if domain == "photography":
        ...
        if camera_type == "sony":
            ...
            if iso >= 1000:
                ...
                read_noise = 3.75  # electrons RMS at ISO 1000 (average of 3.5-4.0 range)
            elif iso >= 200:
        ...
        instrument_params = {
            "ACS_WFC": {"gain": 1.0, "read_noise": 3.5, "method": "fits_header_analysis"},
            "WFC3_UVIS": {"gain": 1.5, "read_noise": 3.09, "method": "fits_header_analysis"},
            "WFC3_IR": {"gain": 2.5, "read_noise": 15.0, "method": "fits_header_analysis"},
            "WFPC2_WF": {"gain": 7.0, "read_noise": 6.5, "method": "fits_header_analysis"},
            "WFPC2_PC": {"gain": 7.0, "read_noise": 6.5, "method": "fits_header_analysis"}
        }
...
def get_calibration_description(domain: str, gain: float, read_noise: float, method: str) -> str:
