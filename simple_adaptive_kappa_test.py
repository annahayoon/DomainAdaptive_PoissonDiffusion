...
def run_simple_test():
    ...
    cmd_constant = [
        "python", "sample/sample_noisy_pt_lle_PGguidance_adaptivekappa.py",
        ...
        "--output_dir", f"{output_dir}/constant_kappa",
        "--domain", "photography",
        "--kappa", "0.8",
        "--sigma_r", "4.0",
        "--num_steps", "15",
    ...
    try:
        result = subprocess.run(cmd_constant, capture_output=True, text=True, timeout=600)
    ...
    cmd_adaptive = [
        ...
        "--noisy_dir", "dataset/processed/pt_tiles/photography/noisy",
        "--clean_dir", "dataset/processed/pt_tiles/photography/clean",
        "--output_dir", f"{output_dir}/adaptive_kappa",
        "--domain", "photography",
        "--kappa", "0.8",
        "--sigma_r", "4.0",
        "--num_steps", "15",
    ...
    try:
        result = subprocess.run(cmd_adaptive, capture_output=True, text=True, timeout=600)
        if result.returncode == 0:
