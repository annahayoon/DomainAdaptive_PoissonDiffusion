...
class OptimizedInferenceRunner:
    ...
    def __init__(self, model_path: str, output_dir: str):
        ...
        self.unified_params = {"kappa": 0.2, "sigma_r": 2.0, "num_steps": 15}
    ...
    def get_test_tiles(
        self, domain: str, sensor_filter: Optional[str] = None
    ) -> List[str]:
    ...
    def run_inference_domain(
        self, domain: str, num_tiles: int = 1000
    ) -> Dict[str, Any]:
        ...
        print(
            f"Parameters: κ={self.unified_params['kappa']}, σ_r={self.unified_params['sigma_r']}, steps={self.unified_params['num_steps']}"
        )
        print(f"{'='*80}")
        ...
        cmd = [
            ...
            domain,
            "--num_examples",
            str(num_tiles),  # Let script select from test split
            "--cross_domain_kappa",
            str(self.unified_params["kappa"]),
            "--cross_domain_sigma_r",
            str(self.unified_params["sigma_r"]),
            "--cross_domain_num_steps",
            str(self.unified_params["num_steps"]),
            "--use_sensor_calibration",
            "--run_methods",
            "pg_x0_cross",  # Only run cross-domain PG method
            "--device",
            "cuda:0",
        ]
    ...
    def aggregate_results(
        self, batch_results: List[Dict[str, Any]], domain: str
    ) -> Dict[str, Any]:
        ...
        if all_tile_results:
            ...
            for tile_result in all_tile_results:
                if (
                    "comprehensive_metrics" in tile_result
                    and "pg_x0_cross" in tile_result["comprehensive_metrics"]
                ):
                    all_metrics.append(
                        tile_result["comprehensive_metrics"]["pg_x0_cross"]
                    )
            ...
            if all_metrics:
                ...
                for key in metric_keys:
                    ...
                    if valid_values:
                        ...
                        aggregate_metrics[f"median_{key}"] = float(
            ...
            if "comprehensive_aggregate_metrics" in results:
                ...
                if "pg_x0_cross" in results["comprehensive_aggregate_metrics"]:
                    ...
                    if "mean_ssim" in metrics:
                        print(
                            f"  SSIM: {metrics['mean_ssim']:.4f} ± {metrics.get('std_ssim', 0):.4f}"
                        )
                    if "mean_psnr" in metrics:
                        print(
                            f"  PSNR: {metrics['mean_psnr']:.2f} ± {metrics.get('std_psnr', 0):.2f} dB"
                        )
                    if "mean_lpips" in metrics:
                        print(
                            f"  LPIPS: {metrics['mean_lpips']:.4f} ± {metrics.get('std_lpips', 0):.4f}"
                        )
                    if "mean_niqe" in metrics:
                        print(
                            f"  NIQE: {metrics['mean_niqe']:.2f} ± {metrics.get('std_niqe', 0):.2f}"
                        )
