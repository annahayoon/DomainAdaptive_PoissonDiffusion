    ...
    try:
        ...
        for metric in metrics:
            if metric in pg_constant.columns and metric in pg_adaptive.columns:
                ...
                print(f"{metric.upper():6s}: Constant {const_mean:.4f} vs Adaptive {adapt_mean:.4f} (Δ={diff:+.4f}) {improvement}")
        ...
        for metric in metrics:
            if metric in pg_constant.columns and metric in pg_adaptive.columns:
                comparison_results['metric_comparison'][metric] = {
                    ...
                    'difference': float(pg_adaptive[metric].mean() - pg_constant[metric].mean())
                }
